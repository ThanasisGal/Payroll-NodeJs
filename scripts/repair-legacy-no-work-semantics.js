...
const CONFIRMATION = 'APPLY_LEGACY_AN_ME_NORMALIZATION';
const invalid = message => { throw Object.assign(new Error(message),
    { code: 'INVALID_REPAIR_COMMAND' }); };
...
function parseArgs(argv = []) {
    ...
    for (let index = 0; index < argv.length; index++) {
        ...
        if (token === '--dry-run') {
            if (args.dryRun) invalid('Duplicate option: --dry-run');
            args.dryRun = true; continue;
        }
        const key = token.startsWith('--') ? token.slice(2) : '';
        if (!values.has(key) || Object.hasOwn(args, key)) {
            invalid(`Unsupported or duplicate option: ${token}`);
        }
    ...
    if (args.apply) {
        for (const key of ['preview-fingerprint', 'reason', 'actor', 'confirm']) {
...
const scopeFromArgs = args => ({ team: args.team, company_kod: args.company,
    ypokatasthma: args.branch, period_start: args['period-start'],
    period_end: args['period-end'] });

async function run(argv, dependencies = {}) {
    const args = parseArgs(argv);
    const service = dependencies.service || require(
        '../server/services/ergazomenoi/apasxoliseisLegacyNoWorkSemanticRepairService');
...
async function main(argv) {
    parseArgs(argv);
    if (!process.env.MONGODB_URL) invalid(
        'MONGODB_URL must already be supplied in the operator environment');
...
if (require.main === module) main(process.argv.slice(2)).catch(error => {
    console.error(error.code || 'LEGACY_NO_WORK_SEMANTIC_REPAIR_FAILED');
    process.exitCode = 1;
});
