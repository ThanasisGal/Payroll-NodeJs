...
echo "=================================================="
echo "🔍 JavaScript Files Audit (EJS-aware)"
echo "=================================================="
echo ""

# 1. Find type="module" in views (handle EJS conditionals)
echo "1️⃣  Scanning for type=\"module\" in views..."
...
echo ""
echo "2️⃣  Scanning for ALL <script src=...> in views..."
...
echo ""
echo "3️⃣  Extracting files from deploy.sh..."
...
echo ""
echo "4️⃣  Finding ALL .js files in public/js/..."
...
echo ""
echo "⚠️  MISSING FROM DEPLOY.SH (used in views but not deployed):"
MISSING=$(comm -23 /tmp/found-all-scripts.txt /tmp/deploy-files.txt)
if [ -z "$MISSING" ]; then
    echo "   ✅ None - all view scripts are in deploy.sh"
else
    echo "$MISSING" | sed 's/^/   ❌ /'
...
echo ""
echo "ℹ️  IN DEPLOY.SH BUT NOT IN VIEWS (imported modules/utilities):"
EXTRA=$(comm -13 /tmp/found-all-scripts.txt /tmp/deploy-files.txt)
if [ -z "$EXTRA" ]; then
    echo "   (None)"
else
    EXTRA_COUNT=$(echo "$EXTRA" | wc -l)
    echo "   Total: $EXTRA_COUNT files (imported by other scripts)"
...
if [ -z "$UNUSED" ]; then
    echo "   ✅ None - all files are deployed"
else
    UNUSED_COUNT=$(echo "$UNUSED" | wc -l)
    echo "   Total: $UNUSED_COUNT files"
    echo "$UNUSED" | head -5 | sed 's/^/   🗑️  /'
...
if [ -n "$MISSING" ]; then
    echo "⚠️  ACTION REQUIRED: Add missing files to deploy.sh"
    echo ""
...
echo "   - /tmp/found-modules.txt"
echo "   - /tmp/found-all-scripts.txt"
echo "   - /tmp/deploy-files.txt"
echo "   - /tmp/all-js-files.txt"
