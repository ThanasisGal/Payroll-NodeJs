...
const { getUserFriendlyError } = require('./errorMessages');
...
// Προσθήκη στην αρχή του αρχείου, μετά τις άλλες helper functions
...
async function clickPrimefacesConfirmYes(apdPage, timeoutMs, widgetVar = 'backConfirmation') {
    ...
    const clicked = await apdPage.evaluate((w) => {
        try {
            ...
            const yes =
                buttons.find((b) => (b.textContent || '').replace(/\s+/g, ' ').trim() === 'Ναι') ||
    ...
    const fallbackClicked = await apdPage.evaluate(() => {
        ...
        for (const d of visibles) {
            const btn = Array.from(d.querySelectorAll('button')).find(
                (b) =>
                    (b.textContent || '').replace(/\s+/g, ' ').trim() === 'Ναι' ||
    ...
    if (!fallbackClicked)
        throw new Error('Δεν κατάφερα να κάνω click στο "Ναι" του ConfirmDialog.');
    return true;
}
...
async function ensureSupplementalPeriodViaDialog2(apdPage, timeoutMs) {
    ...
    if (!(await safeCount(openDialogLink)) && !(await safeCount(dialog)) && !hasRecords)
        return false;
    ...
    if (hasRecords) {
        ...
        if (targetRowIndex === -1) {
            ...
            const safeTd = targetRow
                .locator('td')
                .filter({ hasNot: apdPage.locator('a') })
                .first();
    ...
    await apdPage.selectOption('select#openPeriodForm\\:statementTypeDlg', {
    ...
    const beforeCount = await apdPage
        .locator('#submissionTable_data > tr')
        .count()
        .catch(() => 0);
    await continueBtn.click();

    const pendingMsg = dialog
        .locator('.ui-messages-error, .ui-message-error, .ui-messages-warn, .ui-message-warn')
        .filter({ hasText: 'Υπάρχουν ΑΠΔ' })
        .first();
    ...
    if (isPendingVisible) {
        ...
        if (await safeCount(closeBtn)) {
            ...
            await dialog
                .locator('.ui-dialog-titlebar-close, a[aria-label="Κλείσιμο"]')
                .first()
                .click()
                .catch(() => {});
        }
...
async function continueApd(sessionId, opts = {}) {
    ...
    try {
        ...
        if (await safeCount(consentBtn)) {
            const continueRadio = apdPage.locator(
                'input[type="radio"][name="scope.read"][value="true"]'
            );
        ...
        if (!(await safeCount(amaInput))) {
            ...
            const hasSubmissionTable =
                (await safeCount(apdPage.locator('#submissionTable_data'))) > 0;
            if (hasSubmissionTable) {
                const fillCount = await apdPage
                    .locator('#submissionTable_data a:has-text("Συμπλήρωση")')
                    .count()
                    .catch(() => 0);
                if (fillCount === 0) {
        ...
        if (!hasAmaAlready) {
            await apdPage
                .waitForSelector('#submissionTable_data', { timeout: timeoutMs, state: 'visible' })
                .catch(() => {});
            ...
            for (const i of scanIdxs) {
                ...
                if (
                    bestFrom === null ||
                    cmpMY(to, bestTo) > 0 ||
                    (cmpMY(to, bestTo) === 0 && cmpMY(from, bestFrom) > 0)
                ) {
                    bestIdx = i;
                    bestFrom = from;
                    bestTo = to;
            ...
            const fillLink = bestRow
                .locator('td')
                .nth(4)
                .locator('a:has-text("Συμπλήρωση")')
                .first();
        ...
        const amaRowButton = apdPage
            .locator('input#ama')
            .locator(
                'xpath=ancestor::td[1]//button[contains(@class,"ui-button") and .//span[contains(@class,"ui-icon-search")]]'
            )
            .first();
        ...
        if (await safeCount(criteriaSel)) {
            ...
            if (curVal !== '1') {
                await apdPage
                    .selectOption('select#amaLOVForm\\:criteriaSelector_input', { value: '1' })
                    .catch(() => {});
                await amkaFilter.waitFor({ state: 'visible', timeout: amaTimeout });
            }
        }
        ...
        if (hasEmpty) {
            const emptyText = normalizeText(
                await emptyRow.innerText().catch(() => 'Δεν βρέθηκαν αποτελέσματα.')
            );
        ...
        if (resCount < 1) {
            ...
            throw new Error(
                `0 αποτελέσματα στο amaLOV για ΑΜΚΑ ${amka}. ${raw || 'Δεν βρέθηκε εγγραφή.'}`
            );
        }
        ...
        const amaText = normalizeText(
            await tds
                .nth(0)
                .innerText()
                .catch(() => '')
        );
        const amkaText = normalizeText(
            await tds
                .nth(1)
                .innerText()
                .catch(() => '')
        );
        const afmText = normalizeText(
            await tds
                .nth(2)
                .innerText()
                .catch(() => '')
        );
        const eponymoText = normalizeText(
            await tds
                .nth(3)
                .innerText()
                .catch(() => '')
        );
        const onomaText = normalizeText(
            await tds
                .nth(4)
                .innerText()
                .catch(() => '')
        );
        const patronymoText = normalizeText(
            await tds
                .nth(5)
                .innerText()
                .catch(() => '')
        );
        const mhtronymoText = normalizeText(
            await tds
                .nth(6)
                .innerText()
                .catch(() => '')
        );
        const paliosNeosTxt = normalizeText(
            await tds
                .nth(8)
                .innerText()
                .catch(() => '')
        ...
        await apdPage
            .locator('.ui-dialog:has(#amaLOVForm)')
        ...
        const found = await apdPage.evaluate(() => {
            ...
            const btn =
                ...
                Array.from(topBar.querySelectorAll('button')).find((b) =>
                    (b.textContent || '').includes('Επιστροφή')
                );
        ...
        const cancelLink = apdPage
            .locator('#cancelPeriodLink, a:has-text("Ακύρωση Περιόδου Υποβολής")')
            .first();
