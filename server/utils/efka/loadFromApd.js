    ...
    return withEfkaLock(async () => {
        ...
        try {
            ...
            await context.route('**/*', (route) => {
                ...
                // SAFE blocks (do NOT block stylesheet for PrimeFaces/JSF)
                if (rt === 'image' || rt === 'font' || rt === 'media') {
                    return route.abort();
                }

                // block common analytics (safe)
                if (
            ...
            // wait for a "stable" element of the submissions page instead of generic waits
            // await apdPage.waitForSelector('form#submissionShow', { timeout: 60000 }).catch(() => {});            
