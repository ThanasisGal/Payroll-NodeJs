 ...
 * - extraQueryBuilder: (q) => object     -> επιπλέον φίλτρο από req.query (π.χ. ημερομηνίες)
 * - mapItem:           (item, pad) => {} -> μετατρέπει ένα record σε { value, label, ... }
 * - sort:              object            -> default sort (π.χ. { kodikos: 1 })
 * - baseUrl:           string            -> για nextPage, αλλιώς βγαίνει από το originalUrl
 * - preselectSort:     object|null       -> sort για 2ο γύρο preselect fallback (π.χ. { isxyei_apo: -1 })
 */
...
function createDropdownApi(model, options = {}) {
    const {
        ...
        //     label: `${(String(item[pk] ?? '').trim()).padEnd(pad, '\u00A0')} - ${item.perigrafh ?? ''}`,
        // }),
        mapItem = (item, pad = 11) => {
            const valRaw = item[pk];
            const val    = (valRaw == null) ? '' : String(valRaw).trim();  // <-- string, ποτέ undefined
            const kod    = (item.kodikos == null) ? '' : String(item.kodikos);
            const lab    = `${(val || '').padEnd(pad, '\u00A0')} - ${item.perigrafh ?? ''}`;
            return {
                value    : val,        // <-- ΠΑΝΤΑ string
                kodikos  : kod,
                perigrafh: item.perigrafh,
                label    : lab,
            };
    ...
    return async (req, res) => {
        ...
        try {
            ...
            if (results.length > 0) {
                ...
                //     nextPage: buildNextPageUrl(req, baseUrl, page, skip + results.length < count),
                //     totalCount: count,
                // });
                return res.json({
            ...
            // ---------- 3) Fallback: “πλησιέστερες” εγγραφές ----------
            if (req.query._dateApo && req.query._dateEos) {
                const dateApo = new Date(req.query._dateApo);
                const dateEos = new Date(req.query._dateEos);

                // const pipeline = [
                //     {
                //         $addFields: {
                //             __startGap: { $abs: { $subtract: [{ $toLong: '$isxyei_apo' }, dateApo.getTime()] } },
                //             __endGap: {
                //                 $abs: {
                //                     $subtract: [
                //                         { $toLong: { $ifNull: ['$isxyei_eos', new Date(8640000000000000)] } },
                //                         dateEos.getTime(),
                //                     ],
                //                 },
                //             },
                //         },
                //     },
                //     { $addFields: { __totalGap: { $add: ['$__startGap', '$__endGap'] } } }, // <-- FIXED
                //     // { $sort: { __totalGap: 1, [pk]: 1 } },
                //     {
                //         $facet: {
                ...
                // return res.json({
                // items: data.map((item) => mapItem(item, padLength)),
                // hasMore: skip + data.length < totalCount,
                // nextPage: buildNextPageUrl(req, baseUrl, page, skip + data.length < totalCount),
                // totalCount,
                // });
...
function buildNextPageUrl(req, baseUrl, page, hasMore) {
    ...
    // κράτα τα υπάρχοντα query params εκτός του page
