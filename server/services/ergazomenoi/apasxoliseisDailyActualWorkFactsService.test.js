...
assert.deepStrictEqual(
    ...
    expected({ category: 'ΑΔΕΙΑ', actualWorkHours: 8, leaveHours: 0, sicknessHours: 0, countsAsActualWorkDay: true, reasons: [], warnings: [] })
);
assert.deepStrictEqual(
    facts('ΑΔΕΙΑ', 8, 4, { adeia: true }),
    expected({ category: 'ΑΔΕΙΑ', cardHours: 4, actualWorkHours: 4, leaveHours: 4, sicknessHours: 0, countsAsActualWorkDay: true, reasons: [], warnings: ['MIXED_WORK_AND_HOURLY_LEAVE'] })
);
...
assert.deepStrictEqual(
    ...
    expected({ category: 'ΑΣΘΕΝΕΙΑ', actualWorkHours: 0, leaveHours: 0, sicknessHours: 0, countsAsActualWorkDay: false, reasons: [], warnings: [] })
);
assert.deepStrictEqual(
    facts('ΑΣΘΕΝΕΙΑ', 8, 6, { astheneia: true }),
    expected({ category: 'ΑΣΘΕΝΕΙΑ', cardHours: 6, actualWorkHours: 6, leaveHours: 0, sicknessHours: 2, countsAsActualWorkDay: true, reasons: [], warnings: ['MIXED_WORK_AND_SICKNESS'] })
);
