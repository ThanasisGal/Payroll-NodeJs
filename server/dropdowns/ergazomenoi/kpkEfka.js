...
module.exports = {
  ...
  handler: async (req, res) => {
    try {
      ...
      const eidik = (req.query.eidikothta ?? req.query.kodikos_eidikothtas ?? '').toString().trim();
      if (!kad || !eidik) {
        ...
        { $match: { $expr: {
            $and: [
              { $or: [
                ...
                { $and: [ { $eq: ['$apoEtos', yearInUse] }, { $lte: ['$apoMhnas', periodInUse] } ] }
              ]},
              { $or: [
                { $gt: ['$eosEtos', yearInUse] },
                { $and: [ { $eq: ['$eosEtos', yearInUse] }, { $gte: ['$eosMhnas', periodInUse] } ] }
              ]}
            ]
        }}},
      ...
      if (re) {
        Object.assign(baseMatch, { $or: [{ kodikos: { $regex: re } }, { perigrafh: { $regex: re } }] });
      }
      ...
      const [{ total = 0 } = {}] = await KpkEfkaModel.aggregate([
        ...kpkBase,
        { $count: 'total' }
      ]).collation({ locale: 'el', strength: 1 }).exec();
