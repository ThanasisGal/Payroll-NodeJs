...
static userLogin = async (req, res) => {
    try {
        ...
        if (!user) {
            await res.flash("error", "Μη έγκυρα στοιχεία σύνδεσης");
            return res.render("login/login", { bodyClass: "home-bg-cdn" });
        }
        ...
        // ═══════════════════════════════════════════════════════════
        if (! user.isVerified) {
            await res. flash("error", "Δεν έχετε κάνει επαλήθευση του Email σας.");
            return res.render("login/login", { bodyClass: "home-bg-cdn" });
        }
        ...
        // ═══════════════════════════════════════════════════════════
        // 5. Session Regeneration (Security Critical!)
        // ═══════════════════════════════════════════════════════════
        return req.session.regenerate(async (err) => {
            if (err) {
                console. error('❌ Session regeneration error:', err);
                await res.flash("error", "Σφάλμα ασφαλείας. Προσπαθήστε ξανά.");
                return res. render("login/login", { bodyClass: "home-bg-cdn" });
            }
            ...
            // ═══════════════════════════════════════════════════════════
            // 7. Load User Parameters
            // ═══════════════════════════════════════════════════════════
            let redir = "/companies/genikastoixeia";
