...
const { ProdhlomenaOrariaModel } = Models_Ergazomenon;

class genikaAPIsController {
    static getPerifereies = async (req, res) => {
        try {
            const perifereies = await PerifereiesModel.find()
                .select('kodikos perigrafh')
                .sort('perigrafh'); // Επιλογή των πεδίων που θέλουμε να εμφανιστούν στο dropdown
    ...
    static getNomoi = async (req, res) => {
        try {
            ...
            const nomoi = await NomoiModel.find({ perifereia: perifereia }).sort('perigrafh');
            res.json(nomoi);
        } catch (error) {
            res.status(500).send(error);
        }
    ...
    static getNomikesMorfes = async (req, res) => {
        try {
            const nomikesMorfes = await NomikesMorfesModel.find({}, 'kodikos perigrafh').sort(
                'perigrafh'
            );
    ...
    static getPararthmataEfka = async (req, res) => {
        try {
            const pararthmataEfka = await PararthmataEfkaModel.find({}, 'kodikos perigrafh').sort(
                'perigrafh'
            );
    ...
    static handlePararthmataEfka = async (req, res) => {
        const selectFields = [{ id: 'pararthmaefka', field: 'kodikos', text: 'perigrafh' }];
        try {
    ...
    static checkTexnikosAsfaleias = async (req, res) => {
        try {
            ...
            let aa_kod_ta = null;
            const lastRecord_ta = await TexnikosAsfaleiasModel.find({}).sort({ _id: -1 }).limit(1);

            aa_kod_ta =
                lastRecord_ta[0] && lastRecord_ta[0].kodikos
                    ? parseInt(lastRecord_ta[0].kodikos, 10)
                    : null;
    ...
    static checkIatrosErgasias = async (req, res) => {
        try {
            ...
            let aa_kod_ia = null;
            const lastRecord_ia = await IatrosErgasiasModel.find({}).sort({ _id: -1 }).limit(1);
            aa_kod_ia =
    ...
    static checkLogisths = async (req, res) => {
        try {
            ...
            let aa_kod_lo = null;
            const lastRecord_lo = await LogisthsModel.find({}).sort({ _id: -1 }).limit(1);
            aa_kod_lo =
    ...
    static getDhmoiEditForm = async (req, res) => {
        try {
            ...
            const dhmoi = await DhmoiModel.find({ nomos: nomosKodikos }).sort('perigrafh');
            res.json(dhmoi);
        } catch (error) {
            console.error(error);
    ...
    static getPoleisEditForm = async (req, res) => {
        try {
            ...
            const poleis = await PoleisModel.find({ dhmos: dhmosKodikos }).sort('perigrafh');
            res.json(poleis);
        } catch (error) {
            console.error(error);
    ...
    static getTexnikosAsfaleias = async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ message: 'Σφάλμα κατά την ανάκτηση των τεχνικών ασφαλείας' });
        }
    };
    ...
    static getIatrosErgasias = async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ message: 'Σφάλμα κατά την ανάκτηση των ιατρών εργασίας' });
        }
    };
    ...
    static getEmmesosErgodoths = async (req, res) => {
        try {
            ...
            const emmesoiErgodotes = await EmmesosErgodothsModel.find({
                kodikos: kodikosEmmesoyErgodoth
            }).sort('eponymo');
            res.json(emmesoiErgodotes);
        } catch (error) {
            console.error(error);
            res.status(500).json({ message: 'Σφάλμα κατά την ανάκτηση των Έμμεσων Εργοδοτών' });
        }
    };
    ...
    static getDiadoxosErgodoths = async (req, res) => {
        try {
            ...
            const diadoxoiErgodotes = await DiadoxosErgodothsModel.find({
                kodikos: kodikosDiadoxoyErgodoth
            }).sort('eponymo');
            res.json(diadoxoiErgodotes);
        } catch (error) {
            console.error(error);
            res.status(500).json({ message: 'Σφάλμα κατά την ανάκτηση των Διάδοχων Εργοδοτών' });
        }
    };
    ...
    static getSepe = async (req, res) => {
        try {
            const sepe = await SepeModel.find({}, 'kodikos perigrafh').sort('perigrafh');
            res.json(sepe);
        } catch (error) {
            res.status(500).send(error);
        }
    ...
    static getDypa = async (req, res) => {
        try {
            const dypa = await DypaModel.find({}, 'kodikos perigrafh').sort('perigrafh');
            res.json(dypa);
        } catch (error) {
            res.status(500).send(error);
        }
    ...
    static getIdiothtes = async (req, res) => {
        try {
            const idiothta = await IdiothtesModel.find({}, 'kodikos perigrafh').sort('perigrafh');
            res.json(idiothta);
        } catch (error) {
            res.status(500).send(error);
        }
    ...
    static getNomimosEkprosopos = async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ message: 'Σφάλμα κατά την ανάκτηση των Νόμιμων Εκπροσώπων' });
        }
    };
    ...
    static getBanks = async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ message: 'Σφάλμα κατά την ανάκτηση των Τραπεζών' });
        }
    };

    static getYphkoothtes = async (req, res) => {
        try {
            const yphkoothta = await YphkoothtesModel.find()
                .select('kodikos perigrafh')
                .sort('perigrafh');
            res.json(yphkoothta);
        } catch (error) {
    ...
    static getOresFromEidikesKathgories = async (req, res) => {
        try {
            ...
            const eidikhKathgoria = await EidikesKathgoriesModel.findOne({ kodikos: kodikos }).sort(
                'perigrafh'
            );
            res.json(eidikhKathgoria);
        } catch (error) {
    ...
    static getApasxolhseisBaseiSymbashs = async (req, res) => {
        try {
            const apasxolhshBaseiSymbashs =
                await ApasxolhseisBaseiSymbashsModel.find().sort('kodikos');
    ...
    static getAsfalistikesKlaseis = async (req, res) => {
        ...
        try {
            const documents = await AsfalistikesKlaseisModel.find({ etos: etos_xrhshs }).sort({
                etos: -1,
                kodikos: 1
            });
    ...
    static getEidikothtesEfka = async (req, res) => {
        try {
            ...
            const antistoixies = await AntistoixishKadEidikothtesKpkEfkaModel.find(
                { kodikos_kad: kodikos_kad },
                'kodikos_eidikothtas'
            );
            const kodikoiEidikothtas = antistoixies.map((antistoixia) =>
                antistoixia.kodikos_eidikothtas.toString()
            );
    ...
    static getKpkEfka = async (req, res) => {
        try {
            ...
            const antistoixies = await AntistoixishKadEidikothtesKpkEfkaModel.find(
                { kodikos_kad: kodikos_kad, kodikos_eidikothtas: kodikos_eidikothtas },
                'kodikos_kpk'
            );
            const kodikoiKpk = antistoixies.map((antistoixia) =>
                antistoixia.kodikos_kpk.toString()
            );
    ...
    static getKpk = async (req, res) => {
        try {
            ...
            const kpk = await KpkEfkaModel.find({ kodikos: kodikos_kpkEfka })
                .select('kodikos perigrafh')
                .sort('perigrafh');
            res.json(kpk);
        } catch (error) {
    ...
    static getKpkBaseiEpaEfka = async (req, res) => {
        try {
            ...
            const antistoixies = await AntistoixishEidikhsPeriptoshsKpkEfkaModel.find(
                {
                    kodikos_eidikhs_periptoshs: kodikos_eidikhs_periptoshs,
                    kodikos_kpk_apo: kodikos_kpk_apo
                },
                'kodikos_kpk_se'
            );
            const kodikoiKpk = antistoixies.map((antistoixia) =>
                antistoixia.kodikos_kpk_se.toString()
            );
    ...
    static getKathgoriesSymbaseon = async (req, res) => {
        try {
            ...
            const kathgoriesSymbaseon = await KathgoriesSymbaseonModel.find({
                afora_thn_symbash: symbash
            }).sort('kodikos');
    ...
    static getEidikothtesSymbaseon = async (req, res) => {
        try {
            ...
            const eidikothtesSymbashs = await EidikothtesAnaKathgoriaSymbaseonModel.find({
    ...
    static getStoixeiaSymbaseon = async (req, res) => {
        try {
            ...
            const stoixeiaSymbashs = await StoixeiaSymbaseonModel.find({
                afora_thn_symbash_kathgoria_eidikothta: symbash_kathgoria_eidikothta
            })
                .sort('afora_thn_symbash_kathgoria_eidikothta')
                .lean();
        ...
        try {
            ...
            const klimakiaSymbashs = await KlimakiaSymbaseonModel.find({
                afora_thn_symbash_kathgoria_eidikothta_stoixeio:
                    symbash_kathgoria_eidikothta_stoixeio
            })
                .sort({
    ...
    static getAdeiesDiamonhs = async (req, res) => {
        try {
            ...
            const adeiesDiamonhs = await AdeiesDiamonhsModel.find({ typos: typosAdeias }).sort(
                'perigrafh'
            );
    ...
    static getThematikaPedia = async (req, res) => {
        try {
            const thematikaPedia = await ThematikaPediaModel.find({}, 'kodikos perigrafh').sort(
                'perigrafh'
            ); // Επιλογή των πεδίων που θέλουμε να εμφανιστούν στο dropdown
    ...
    static getThematikesEnothtes = async (req, res) => {
        try {
            ...
            const thematikesEnothtes = await ThematikesEnothtesModel.find({
                kodikos_sysxetishs: thematikoPedio
            }).sort('perigrafh');
    ...
    static getProdhlomenaOraria = async (req, res) => {
        try {
            const { kodikos, startDate, endDate } = req.body;

            // ✅ team και company_kod από το session (ΔΕΝ τα στέλνει ο client - ασφαλές!)
            const team = req.session.userTeam;
            const company_kod = req.session.companyInUse;

            if (!team || !company_kod || !kodikos || !startDate || !endDate) {
                return res.status(400).json({ error: 'Λείπουν απαραίτητα πεδία' });
            }

            // ✅ Ασφαλής parsing χωρίς timezone πρόβλημα
            // Κόβουμε απλώς το string π.χ. "2026-03-16" → parts[0]=2026, [1]=03, [2]=16
            const startParts = startDate.split('-');
            const endParts = endDate.split('-');

            // ✅ Date.UTC → πάντα UTC, ανεξάρτητα από το timezone του server
            ...
            const end = new Date(
                Date.UTC(
                    ...
                    59,
                    59,
                    999 // 23:59:59.999 UTC
                )
            );

            const records = await ProdhlomenaOrariaModel.find({
                team: team,
            ...
            console.error('❌ Σφάλμα στο getProdhlomenaOraria:', error);
