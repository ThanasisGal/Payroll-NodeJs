...
class programmataController {
    ...
    static deleteOrariaErgazomenoyApoEos = async (req, res) => {
        try {
            ...
            console.log(
                'Error into programmataController -> deleteOrariaErgazomenoyApoEos :',
                error
            );
        }
    };
    ...
    static mainAntigrafhProgrammatonForm = async (req, res) => {
        ...
        try {
            ...
            console.log(
                'Error into programmataController -> mainAntigrafhProgrammatonForm :',
                error
            );
        }
    };
        ...
        try {
            ...
            const schedules = await OrariaModel.find({
                ...
                hmeromhnia: mongoose.trusted({
                    $gte: new Date(fromStartDate),
                    $lte: new Date(fromEndDate)
                })
            });
            ...
            const updatePromises = schedules.map(async (schedule) => {
                ...
                for (
                    let currentDate = new Date(toStart);
                    currentDate.toISOString() <= toEnd.toISOString();
                    ...
                    if (currentDate.getDay() === dayOfWeek) {
                        ...
                        const isHoliday = await ArgiesModel.findOne({
                            hmeromhnia: adjustedCurrentDate
                        });
            ...
            try {
                const orariaSErgazomenou = await OrariaModel.find({
                    ...
                    kodikos: toSelectedKodikos,
                    hmeromhnia: mongoose.trusted({
                        $gte: new Date(toStartDate),
                        $lte: new Date(toEndDate)
                    })
                }).sort({ hmeromhnia: 1 });
                ...
                for (let i = 0; i < intervals.length; i++) {
                    ...
                    if (weeklyTotal <= 40) {
                        ...
                        for (let j = fromDay; j <= toDay; j++) {
                            let ores_ergasias =
                                bulkOps[j].updateOne.update.$inc.ores_ergasias +
                                    ...
                                    bulkOps[j].updateOne.update.$inc
                                        .ores_yperergasias_argion_nyxtas || 0;

                            let ores_nyxtas =
                                bulkOps[j].updateOne.update.$inc.ores_nyxtas +
                                    bulkOps[j].updateOne.update.$inc.ores_yperergasias_nyxtas +
                                    bulkOps[j].updateOne.update.$inc
                                        .ores_yperergasias_argion_nyxtas || 0;

                            let ores_argion =
                                bulkOps[j].updateOne.update.$inc.ores_argion +
                                    bulkOps[j].updateOne.update.$inc.ores_yperergasias_argion +
                                    bulkOps[j].updateOne.update.$inc
                                        .ores_yperergasias_argion_nyxtas || 0;
    ...
    static getOraria = async (req, res) => {
        ...
        try {
            ...
            const oraria = await OrariaModel.find(
                {
                    ...
                    hmeromhnia: mongoose.trusted({
                        $gte: new Date(apoHmeromhnia),
                        $lte: new Date(eosHmeromhnia)
                    })
                },
                {
                    team: 1,
            ...
            const employees = await ErgazomenoiModel.find(
                { team: team, company_kod: company, kodikos: mongoose.trusted({ $in: kodikoi }) },
            ...
            oraria.forEach((orario) => {
                ...
                const employeeKey =
                    filetype === 'xlsx' && diadikasia === '182' ? `${afm}` : `${afm}-${dateKey}`;
                ...
                if (orario) {
                    ...
                    combinedResults[employeeKey].analytics.push({
                        ...
                        ...(diadikasia === '182' && filetype === 'xlsx'
                            ? {
                                  days: {
                                      [dayName]: `${times1.from.replace(':', '')}${times1.to.replace(':', '')}`
                                  }
                              }
                            : {})
                    });
                ...
                if (orario.apo_ora_02 && orario.eos_ora_02) {
                    ...
                    const lastEntry =
                        combinedResults[employeeKey].analytics[
                            combinedResults[employeeKey].analytics.length - 1
                        ];
                    if (lastEntry.days && lastEntry.days[dayName]) {
                        // Συνδυασμός των ωρών για την ίδια ημέρα
                        lastEntry.days[dayName] +=
                            `, ${times2.from.replace(':', '')}${times2.to.replace(':', '')}`;
                    } else {
                        combinedResults[employeeKey].analytics.push({
                            ...
                            ...(diadikasia === '182' && filetype === 'xlsx'
                                ? {
                                      days: {
                                          [dayName]: `${times2.from.replace(':', '')}${times2.to.replace(':', '')}`
                                      }
                                  }
                                : {})
                        });
                ...
                if (orario.apo_ora_03 && orario.eos_ora_03) {
                    ...
                    combinedResults[employeeKey].analytics.push({
                        ...
                        ...(diadikasia === '182' && filetype === 'xlsx'
                            ? {
                                  days: {
                                      [dayName]: `${times3.from.replace(':', '')}${times3.to.replace(':', '')}`
                                  }
                              }
                            : {})
                    });
                        ...
                        case '182':
                            ...
                            function getFTypeForDay(analytics, dayName) {
                                const analytic = analytics.find(
                                    (analytic) =>
                                        analytic.days && analytic.days[dayName] !== undefined
                                );
                                if (
                                    analytic &&
                                    (analytic.f_type === 'ΑΝ' || analytic.f_type === 'ΜΕ')
                                ) {
                            ...
                            Object.values(combinedResults).forEach((item) => {
                                ...
                                const type = item.analytics.some(
                                    (analytic) => analytic.f_type === 'ΤΗΛ'
                                )
                                    ? 'ΤΗΛ'
                                    : 'ΕΡΓ';
                                ...
                                const morningRow = [
                                    ...
                                    item.onoma,
                                    type,
                                    dayRows.morning['ΔΕΥΤΕΡΑ'] ||
                                        getFTypeForDay(item.analytics, 'ΔΕΥΤΕΡΑ'),
                                    dayRows.morning['ΤΡΙΤΗ'] ||
                                        getFTypeForDay(item.analytics, 'ΤΡΙΤΗ'),
                                    dayRows.morning['ΤΕΤΑΡΤΗ'] ||
                                        getFTypeForDay(item.analytics, 'ΤΕΤΑΡΤΗ'),
                                    dayRows.morning['ΠΕΜΠΤΗ'] ||
                                        getFTypeForDay(item.analytics, 'ΠΕΜΠΤΗ'),
                                    dayRows.morning['ΠΑΡΑΣΚΕΥΗ'] ||
                                        getFTypeForDay(item.analytics, 'ΠΑΡΑΣΚΕΥΗ'),
                                    dayRows.morning['ΣΑΒΒΑΤΟ'] ||
                                        getFTypeForDay(item.analytics, 'ΣΑΒΒΑΤΟ'),
                                    dayRows.morning['ΚΥΡΙΑΚΗ'] ||
                                        getFTypeForDay(item.analytics, 'ΚΥΡΙΑΚΗ')
                                ];

                                worksheetData.push(morningRow);

                                const hasAfternoon = Object.values(dayRows.afternoon).some(
                                    (value) => value
                                );
                                if (hasAfternoon) {
                                    const afternoonRow = [
                ...
                case 'xml':
                    ...
                    switch (diadikasia) {
                        case '183':
                            ...
                            const formattedFromDate = new Date(apoHmeromhnia).toLocaleDateString(
                                'el-GR',
                                {
                                    day: '2-digit',
                                    month: '2-digit',
                            ...
                            const formattedToDate = new Date(eosHmeromhnia).toLocaleDateString(
                                'el-GR',
                                {
                                    day: '2-digit',
                                    month: '2-digit',
                            ...
                            xmlDoc = xmlbuilder
                                ...
                                .ele('f_comments')
                                .up()
                                .ele('f_from_date', formattedFromDate)
                                .up()
                                .ele('f_to_date', formattedToDate)
                                .up();
                            ...
                            Object.keys(combinedResults).forEach((key) => {
                                ...
                                const formattedDate = new Date(item.hmeromhnia).toLocaleDateString(
                                    'el-GR',
                                    {
                                        day: '2-digit',
                                        month: '2-digit',
                                ...
                                const employee = ergazomenoiElement
                                    .ele('ErgazomenoiWTO')
                                    .ele('f_afm', item.afm)
                                    .up()
                                    .ele('f_eponymo', item.eponymo)
                                    .up()
                                    .ele('f_onoma', item.onoma)
                                    .up()
                                    .ele('f_date', formattedDate)
                                    .up()
                                    .ele('ErgazomenosAnalytics');

                                // Εισάγουμε όλα τα analytics για τον εργαζόμενο
                                item.analytics.forEach((analytic) => {
                                    const employeeAnalytics = employee
                                        .ele('ErgazomenosWTOAnalytics')
                                        .ele('f_type', analytic.f_type)
                                        .up();
                        ...
                        case '182':
                            ...
                            xmlDoc = xmlbuilder
                                ...
                                .ele(
                                    'f_from_date',
                                    new Date(apoHmeromhnia).toLocaleDateString('el-GR', {
                                        day: '2-digit',
                                        month: '2-digit',
                                        year: 'numeric'
                                    })
                            ...
                            Object.keys(combinedResults).forEach((key) => {
                                ...
                                const employee = ergazomenoiElement
                                    .ele('ErgazomenoiWTO')
                                    .ele('f_afm', item.afm || '999999999')
                                    .up()
                                    .ele('f_eponymo', item.eponymo || 'ΤΕΣΤ')
                                    .up()
                                    .ele('f_onoma', item.onoma || 'ΤΕΣΤ')
                                    .up()
                                    .ele('f_day', dayOfWeek)
                                    .up() // Ημέρα της εβδομάδας
                                    .ele('ErgazomenosAnalytics');
                                ...
                                item.analytics.forEach((analytic) => {
                                    ...
                                    employee
                                        .ele('ErgazomenosWTOAnalytics')
                                        .ele('f_type', analytic.f_type || 'ΑΝ')
                                        .up()
                                        .ele('f_from', times.from)
                                        .up() // Ώρες σε μορφή HH:MM για το XML
                                        .ele('f_to', times.to)
                                        .up(); // Ώρες σε μορφή HH:MM για το XML
                                });
                            });
                                    ...
                                    WTO: [
                                        {
                                            ...
                                            f_rel_date: '',
                                            f_comments: 'test',
                                            f_from_date: new Date(apoHmeromhnia).toLocaleDateString(
                                                'el-GR',
                                                {
                                                    day: '2-digit',
                                                    month: '2-digit',
                                            ...
                                            f_to_date: new Date(eosHmeromhnia).toLocaleDateString(
                                                'el-GR',
                                                {
                                                    day: '2-digit',
                                                    month: '2-digit',
                                            ...
                                            Ergazomenoi: Object.values(combinedResults).map(
                                                (item) => ({
                                                    f_afm: item.afm,
                                                    ...
                                                    ErgazomenosAnalytics: {
                                                        ErgazomenosWTOAnalytics: item.analytics.map(
                                                            (analytic) => ({
            ...
            res.json({
                success: true,
                redirectUrl: '/ergazomenoi/programmata/exagoghOrarionSeErganh',
        ...
        async function uploadOrariaXml(username, password) {
            ...
            try {
                ...
                await driver
                    .findElement(
                        By.id('ctl00_ctl00_ContentHolder_ContentHolder_SiteLogin_UserName')
                    )
                    .sendKeys(username);
                await driver
                    .findElement(
                        By.id('ctl00_ctl00_ContentHolder_ContentHolder_SiteLogin_Password')
                    )
                ...
                await driver
                    .findElement(
                        By.xpath(
                            '//*[@id="ctl00_ctl00_ContentHolder_ContentHolder_SdcTableMenu"]/div/ol/li[8]/ol/li[1]/ol/li[1]/div/a'
                        )
                    )
                    .click();
                ...
                const option = await dropdown.findElement(
                    By.css(`option[value="${_DIADIKASIA_YPOBOLHS}"]`)
                );
                await option.click();
                ...
                await driver.executeScript(
                    "document.getElementById('ctl00_ctl00_ContentHolder_ContentHolder_AnaggeliesXMLControl_SubmitBox').checked = true;"
                );
...
module.exports = programmataController;
