...
function padText(s, w) {
    return (s ?? '').toString().padEnd(w, '\u00A0');
}
...
function injectMonthRangeFromSession(req, _res, next) {
    ...
    const month = String(req.session?.periodInUse || '')
        .trim()
        .padStart(2, '0');
...
router.get(
    '/ypokatasthmata/pararthmataEfkaErgolaboy',
    buildDropdownRoute(PararthmataEfkaModel, pararthmataEfkaErgolaboy.options)
);
...
router.get(
    '/nomimoiEkprosopoi/taytothta',
    buildDropdownRoute(TypoiTaytothtonModel, taytothtes.options)
);
...
router.get(
    '/ergazomenoi/eidikh_kathgoria',
    buildDropdownRoute(EidikesKathgoriesModel, eidikesKathgories.options)
);
router.get(
    '/ergazomenoi/oikogeneiakh_katastash',
...
router.get(
    '/ergazomenoi/kathestos_apasxolhshs',
    buildDropdownRoute(KathestosApasxolhshsModel, kathestosApasxolhseon.options)
);
router.get(
    '/ergazomenoi/sxesh_ergasias',
    buildDropdownRoute(SxeseisErgasiasModel, sxeseisErgasias.options)
);
router.get(
    '/ergazomenoi/syggenikh_sxesh',
    buildDropdownRoute(SyggenikesSxeseisModel, syggenikesSxeseis.options)
);
router.get(
    '/ergazomenoi/thesh_eythynhs',
    buildDropdownRoute(TheseisEythynhsModel, theseisEythynhs.options)
);
router.get(
    '/ergazomenoi/eidikh_periptosh',
    buildDropdownRoute(EidikesPeriptoseisModel, eidikesPeriptoseis.options)
);
router.get(
    '/ergazomenoi/apasxolhsh_basei_symbashs',
    buildDropdownRoute(ApasxolhseisBaseiSymbashsModel, apasxolhseisBaseiSymbashs.options)
);

router.get(
    '/ergazomenoi/asfalistikh_klash',
    ...
    buildDropdownRoute(AsfalistikesKlaseisModel, {
        ...
        mapItem: (it, pad = 2) => {
            ...
            const n = (v, dec = 2, w = 7) =>
                (v == null ? '' : Number(v).toFixed(dec)).toString().padStart(w, NBSP);
...
router.get(
    '/ergazomenoi/ekpaideytiko_epipedo',
    buildDropdownRoute(EkpaideytikoEpipedoModel, ekpaideytikaEpipeda.options)
);
router.get(
    '/ergazomenoi/eidikothta',
    buildDropdownRoute(EidikothtesEfarmoghsModel, eidikothtes.options)
);
router.get(
    '/ergazomenoi/typos_ergazomenon',
    buildDropdownRoute(TypoiErgazomenonModel, typoiErgazomenon.options)
);
router.get(
...
router.get(
    '/ergazomenoi/foreas_kyrias_asfalishs',
    buildDropdownRoute(ForeasKyriasAsfalishsModel, foreas_kyrias_asfalishs.options)
);
router.get(
...
router.get(
    '/ergazomenoi/meiosh_ergatikhs_eisforas',
    buildDropdownRoute(TypoiEpidothseonModel, meioseisEisforonErgazomenon.options)
);
router.get(
    '/ergazomenoi/epidothsh_ergodotikhs_eisforas',
    buildDropdownRoute(TypoiEpidothseonModel, epidothseisEisforonErgodoth.options)
);
router.get(
    '/ergazomenoi/meiosh_eisforas_mhteron',
    buildDropdownRoute(TypoiEpidothseonModel, meioseisEisforonMhteron.options)
);
...
router.get(
    '/ergazomenoi/kentro_kostoys',
    buildDropdownRoute(KentraKostoysModel, kentraKostoys.options)
);
...
router.get(
    '/ergazomenoi/epikoyrikh_xoris_efka',
    buildDropdownRoute(ForeasEpikoyrikhsXorisEfkaModel, foreas_epikoyrikhs_xoris_efka.options)
);
router.get(
    '/ergazomenoi/astheneia_xoris_efka',
    buildDropdownRoute(ForeasAstheneiasXorisEfkaModel, foreas_astheneias_xoris_efka.options)
);
router.get(
    '/ergazomenoi/idiothta_sto_ergo_39',
    buildDropdownRoute(IdiothtaErgoy39Par9Model, idiothta_antisymbal_sto_ergo_39.options)
);
...
router.get(
    '/ergazomenoi/adeies_diamonhs_typos_0',
    buildDropdownRoute(AdeiesDiamonhsModel, adeies_diamonhs_typos_0.options)
);
// ✅ Άδειες Διαμονής με typos = "1"
router.get(
    '/ergazomenoi/adeies_diamonhs_typos_1',
    buildDropdownRoute(AdeiesDiamonhsModel, adeies_diamonhs_typos_1.options)
);
...
// ✅ Άδειες Διαμονής με typos = "0"
router.get(
    '/ergazomenoi/thematiko_pedio',
    buildDropdownRoute(ThematikaPediaModel, thematika_pedia.options)
);
router.get(
    '/ergazomenoi/thematikh_enothta',
    buildDropdownRoute(ThematikesEnothtesModel, thematikes_enothtes.options)
);
router.get(
    '/ergazomenoi/foreas_katartishs',
    buildDropdownRoute(ForeisEkpaideyshsModel, foreis_katartishs.options)
);
...
router.get(
    '/ergazomenoi/oysiodeis_oroi',
    buildDropdownRoute(ApodoxhOysiodonOronModel, oysiodeis_oroi.options)
);
...
router.get(
    '/symbaseis/kathgories',
    buildDropdownRoute(KathgoriesSymbaseonModel, kathgoriesSymbaseon.options)
);
router.get('/symbaseis/kathgories', symbaseisController.listKathgoriesSymbaseon);
router.get(
    '/symbaseis/kathgoria_symbashs',
    buildDropdownRoute(KathgoriesSymbaseonModel, kathgoriesApoSymbaseis.options)
);
router.get(
    '/symbaseis/eidikothta_symbashs',
    buildDropdownRoute(EidikothtesAnaKathgoriaSymbaseonModel, eidikothtesApoKathgories.options)
);
router.get(
    '/symbaseis/eidikothta_symbashs_multi',
    buildDropdownRoute(EidikothtesAnaKathgoriaSymbaseonModel, eidikothtesApoKathgoriesMulti.options)
);
router.get(
    '/symbaseis/stoixeio_symbashs',
    buildDropdownRoute(StoixeiaSymbaseonModel, stoixeiaApoEidikothtes.options)
);
