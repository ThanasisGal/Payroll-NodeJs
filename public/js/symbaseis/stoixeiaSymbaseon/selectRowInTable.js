...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tableBody.addEventListener("click", function (event) {
    ...
    if (targetRow.classList.contains("selected-row")) {
      ...
      document.getElementById("edit-btn").href = "/symbaseis/stoixeiaSymbaseon/edit/" + selectedRowId;
    }
  });
  ...
  document.getElementById("delete-btn").addEventListener("click", async function (event) {
    if (!selectedRowId) {
      Swal.fire({
        icon: "error",
        title: "Προσοχή!",
        text: "Παρακαλώ επιλέξτε ένα στοιχείο σύμβασης πρώτα.",
    ...
    var deleteUrl = "/symbaseis/stoixeiaSymbaseon/delete/" + selectedRowId;

    const result = await Swal.fire({
      title: "Είστε σίγουρος/η;",
      html: `ΠΡΟΣΟΧΗ!!! Δεν θα μπορείτε να αναιρέσετε αυτή την ενέργεια! <br> Με την διαγραφή του στοιχείου σύμβασης θα διαγραφούν, (<strong>ΟΛΕΣ</strong>) οι εγγραφές των κλιμακίων, που αφορούν το συγκεκριμένο στοιχείο σύμβασης.`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          Swal.fire({
            icon: "success",
            title: "Επιτυχής Διαγραφή του Στοιχείου Σύμβασης",
            html: tableHtml,
        ...
        Swal.fire({
          icon: "error",
          title: "Σφάλμα κατά τη Διαγραφή του Στοιχείου Σύμβασης",
          text: error.toString(),
          confirmButtonText: "Κλείσιμο",
          customClass: {
