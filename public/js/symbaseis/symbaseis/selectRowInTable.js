  ...
  rows.forEach(function (row) {
    row.addEventListener("click", function () {
      if (this.classList.contains("selected-row")) {
        ...
        document.getElementById("edit-btn").href = "/symbaseis/symbaseis/edit/" + selectedRowId;
      }
    });
  });
  ...
  document.getElementById("delete-btn").addEventListener("click", async function (event) {
    if (!selectedRowId) {
      Swal.fire({
        icon: "error",
        title: "Προσοχή!",
        text: "Παρακαλώ επιλέξτε μια σύμβαση πρώτα.",
    ...
    var deleteUrl = "/symbaseis/symbaseis/delete/" + selectedRowId;

    const result = await Swal.fire({
      title: "Είστε σίγουρος/η;",
      html: `ΠΡΟΣΟΧΗ!!! Δεν θα μπορείτε να αναιρέσετε αυτή την ενέργεια! <br> Με την διαγραφή της σύμβασης θα διαγραφούν (<strong>απ' όλα τα αρχεία</strong>) οι εγγραφές, που αφορούν την συγκεκριμένη σύμβαση.`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          Swal.fire({
            icon: "success",
            title: "Επιτυχής Διαγραφή της Σύμβασης",
            html: tableHtml,
          ...
          Swal.fire({
            icon: "info",
            title: "Δεν υπήρχαν εγγραφές προς διαγραφή.",
            confirmButtonText: "Εντάξει",
            customClass: {
        ...
        Swal.fire({
          icon: "error",
          title: "Σφάλμα κατά τη Διαγραφή της Σύμβασης",
          text: error.toString(),
          confirmButtonText: "Κλείσιμο",
          customClass: {
