...
//         if (!wrap) return;
//         wrap.querySelectorAll('.ts-inline-trash, [class^="ts-inline-trash-"]').forEach((n) =>
//             n.remove()
//         );
//     };
...
//             const symIcon = symRow?.querySelector(
//                 '.bi-trash3, .ts-ext-trash-symbash, [class*="trash"] svg'
//             );
//             if (symRow && symIcon) {
...
//             const size = Math.min(
//                 SIZE_MAX_PX,
//                 Math.max(SIZE_MIN_PX, Math.round(base * SIZE_SCALE))
//             );
...
//                 pointerEvents: 'auto'
//             });
//             btn.tabIndex = -1; // property
//             btn.setAttribute('tabindex', '-1'); // (προαιρετικό αλλά καλό για σιγουριά)
...
//                 target.closest &&
//                 target.closest(
//                     '.bi-trash3, .ts-ext-trash-symbash, .ts-single-reset-btn, [class*="trash"] svg, [class*="trash"]'
//                 );
...
(() => {
    ...
    function mountRowTrash(selectId, { title, onClick }) {
        const sel = $(selectId);
        if (!sel) return;
        if (isMulti(sel)) return; // ✅ ΜΗΝ βάζεις trash σε multiple selects
