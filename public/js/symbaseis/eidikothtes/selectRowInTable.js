...
document.addEventListener("DOMContentLoaded", function () {
    // ---------------- Πίνακας ----------------
    const table =
      document.getElementById("eidikothtes") ||
	...
	tableBody.addEventListener("click", function (event) {
		...
		// αν ήταν ήδη επιλεγμένη → αποεπιλογή
		if (isAlreadySelected) {
			targetRow.classList.remove("selected-row");
			selectedRowId = null;
			updateActionHrefs();   // θα ξανακάνει editBtn.href = "#"
			return;
		}

		// αλλιώς κανονική επιλογή (όπως πριν)
