...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tableBody.addEventListener("click", function (event) {
    ...
    if (targetRow.classList.contains("selected-row")) {
      ...
      document.getElementById("edit-btn").href = "/symbaseis/kathgories/edit/" + selectedRowId;
    }
  });
  ...
  document.getElementById("delete-btn").addEventListener("click", async function (event) {
    if (!selectedRowId) {
      Swal.fire({
        icon: "error",
        title: "Προσοχή!",
        text: "Παρακαλώ επιλέξτε μια κατηγορία σύμβασης πρώτα.",
    ...
    var deleteUrl = "/symbaseis/kathgories/delete/" + selectedRowId;

    const result = await Swal.fire({
      title: "Είστε σίγουρος/η;",
      html: `ΠΡΟΣΟΧΗ!!! Δεν θα μπορείτε να αναιρέσετε αυτή την ενέργεια! <br> Με την διαγραφή της κατηγορίας σύμβασης θα διαγραφούν, (<strong>απ' όλα τα αρχεία</strong>) οι εγγραφές, που αφορούν την συγκεκριμένη κατηγορία σύμβασης.`,
      icon: "warning",
