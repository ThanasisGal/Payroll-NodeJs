...
async function ypologismosPragmatikonErgasimonHmeronMhna(
    year,
    month,
    hmeres,
    genikesParametroi,
    ...
    if (
        hmeromhnia_proslhpshs &&
        new Date(hmeromhnia_proslhpshs).toISOString() >= startDate &&
        new Date(hmeromhnia_proslhpshs).toISOString() <= endDate
    ) {
        startDate = new Date(hmeromhnia_proslhpshs).toISOString();
    }
    if (
        hmeromhnia_apoxorhshs &&
        new Date(hmeromhnia_apoxorhshs).toISOString() >= startDate &&
        new Date(hmeromhnia_apoxorhshs).toISOString() <= endDate
    ) {
    ...
    for (let day = apoDate; day <= eosDate; day.setDate(day.getDate() + 1)) {
        ...
        if (!isHoliday && !xarakthrismos_ergazomenoy && _PLHROMH_ERGATON_TIS_ARGIES === 'Ο')
            isHoliday = true;
...
const allowedKeys = [
    // ΠΛΗΚΤΡΑ ΠΟΥ ΕΠΙΤΡΕΠΕΤΑΙ ΝΑ ΧΡΗΣΙΜΟΠΟΙΘΟΥΝ ΣΕ ΑΡΙΘΜΗΤΙΚΑ ΠΕΔΙΑ
    '0',
    '1',
    '2',
    '3',
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function applySharedParamsCalculationContext(sharedParams) {
        ...
        _ORES_HMERHSIAS_ERGASIAS =
            parseFloat(sharedParams.ergazomenoi.ores_ergasias_ebdomadas) /
            parseFloat(sharedParams.ergazomenoi.hmeres_ergasias_ebdomadas);
        // ΕΡΓΑΣΙΑ ΒΑΣΕΙ ΣΥΜΒΑΣΗΣ
        sharedParams._PROSAYXHSH_HMERON_5MERHS_ERGASIAS =
            sharedParams.ergazomenoi.apasxolhsh_basei_symbashs !== '5'
                ? 1
                : (
    ...
    async function runHmeresErgasiasCalculationCore() {
        ...
        // Η παλιά κλήση /api/kinhseis/calcApoysies έχει ακυρωθεί.
        // Οι ημέρες/ώρες απουσίας φορτώνονται πλέον μαζί με τα υπόλοιπα totals
        // από ProdhlomenaOrariaModel μέσω fetchCalcTotals/getTotalValues.
    ...
    async function calcOresErgasias() {
        _APODOXES_ORON_ERGASIAS =
            parseFloat(document.getElementById('oresErgasias').value) * pragmatikoOromisthio.value;
        await finishValueCalculation();
    }
    ...
    oresApoysias.addEventListener('input', async (event) => {
        if (shouldIgnoreFieldEventDuringLoad()) return;
        _APODOXES_ORON_APOYSIAS =
            parseFloat(document.getElementById('oresApoysias').value) * pragmatikoOromisthio.value;
        await finishValueCalculation();
    });
    ...
    window.calcHmeresErgasiasMeionApoysies = async function () {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΗΜΕΡΩΝ ΕΡΓΑΣΙΑΣ ΜΕΙΟΝ ΑΠΟΥΣΙΕΣ
        if (hasRecord) return;
        ...
        hmeresErgasiasMeionApoysies.value = parseFloat(
            parseFloat(hmeresErgasias.value) -
                parseFloat(hmeresApoysias.value) *
                    (parseFloat(sharedParams.genikesParametroi[9].timh) /
                        parseFloat(sharedParams.genikesParametroi[4].timh)) -
                hmeresAstheneiasValue
        ).toFixed(2);

        if (!_CHECK_HMERES_ASFALISHS) {
            hmeresAsfalishs.value = Math.min(
                parseFloat(sharedParams.genikesParametroi[5].timh),
                Math.round(
                    parseFloat(hmeresErgasiasMeionApoysies.value) +
                        parseFloat(hmeresAstheneias.value)
                )
            );
        }
    ...
    async function calcSynoloAsfalistikhsAxias() {
        ...
        synolo_asfalistikhs_axias = parseFloat(
            parseFloat(document.getElementById('asfalistikhAxiaArgion').value) +
                ...
                parseFloat(
                    document.getElementById('asfalistikhAxiaYperergasiasArgionNyxtas').value
                ) +
                ...
                parseFloat(
                    document.getElementById('asfalistikhAxiaNomimhsYperoriasArgionNyxtas').value
                ) +
                parseFloat(document.getElementById('asfalistikhAxiaParanomhsYperorias').value) +
                parseFloat(
                    document.getElementById('asfalistikhAxiaParanomhsYperoriasArgion').value
                ) +
                parseFloat(
                    document.getElementById('asfalistikhAxiaParanomhsYperoriasNyxtas').value
                ) +
                parseFloat(
                    document.getElementById('asfalistikhAxiaParanomhsYperoriasArgionNyxtas').value
                )
        ).toFixed(2);
    ...
    async function calcAxiaArgion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΑΡΓΙΩΝ
        if (hasRecord) return;
        ...
        let oromisthio =
            ypologismos !== 'Π'
                ? parseFloat(nomimoOromisthio?.value || 0)
                : parseFloat(pragmatikoOromisthio?.value || 0);
        ...
        let prosayxhsh =
            sharedParams.ergazomenoi.plhrhs_apasxolhsh && sharedParams.ergazomenoi.karta_ergasias
                ? axia
                : 0;
    ...
    async function calcAxiaNyxterinon() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΝΥΧΤΕΡΙΝΩΝ
        if (hasRecord) return;
        ...
        if (sharedParams.ergazomenoi.eidikh_kathgoria_ergazomenoy !== '0017') {
            // Αποκλειστικά Νυχτερινή Εργασία
            // Τελικός Υπολογισμός
            let axia = oromisthio * timhPercent * oresNyxtas || 0;
            let prosayxhsh =
                sharedParams.ergazomenoi.plhrhs_apasxolhsh &&
                sharedParams.ergazomenoi.karta_ergasias
                    ? axia
                    : 0;
            let asfalistikhAxia = prosayxhsh;
            ...
            if (
                parseFloat(sharedParams.ergazomenoi.pragmatikosMisthos) >
                parseFloat(sharedParams.ergazomenoi.nomimosMisthos)
            ) {
                let pososto_yperbashs_pragmatikoy_misthoy = parseFloat(
                    (parseFloat(sharedParams.ergazomenoi.pragmatikosMisthos) -
                        parseFloat(sharedParams.ergazomenoi.nomimosMisthos)) /
                        parseFloat(sharedParams.ergazomenoi.nomimosMisthos)
                ).toFixed(4);

                let neo_pososto_nyxterinon =
                    parseFloat(pososto_yperbashs_pragmatikoy_misthoy) >= timhPercent
                        ? timhPercent
                        : parseFloat(
                              timhPercent - parseFloat(pososto_yperbashs_pragmatikoy_misthoy)
                          ).toFixed(4);
    ...
    async function calcAxiaYperergasion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΥΠΕΡΕΡΓΑΣΙΩΝ
        if (hasRecord) return;
    ...
    async function calcAxiaYperergasionNyxtas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΥΠΕΡΕΡΓΑΣΙΩΝ ΝΥΧΤΑΣ
        if (hasRecord) return;

        let oresYperergasiasNyxtas = parseFloat(
            document.getElementById('oresYperergasiasNyxtas').value
        );
        let axiaYperergasiasNyxtas = document.getElementById('axiaYperergasiasNyxtas');
        let asfalistikhAxiaYperergasiasnNyxtas = document.getElementById(
            'asfalistikhAxiaYperergasiasNyxtas'
        );
        ...
        let axia =
            (oromisthio_nyxtas + oromisthio_nyxtas * timhPercent) * oresYperergasiasNyxtas || 0;
    ...
    async function calcAxiaYperergasionArgion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΥΠΕΡΕΡΓΑΣΙΩΝ ΑΡΓΙΩΝ
        if (hasRecord) return;

        let oresYperergasiasArgion = parseFloat(
            document.getElementById('oresYperergasiasArgion').value
        );
        let axiaYperergasiasArgion = document.getElementById('axiaYperergasiasArgion');
        let asfalistikhAxiaYperergasiasArgion = document.getElementById(
            'asfalistikhAxiaYperergasiasArgion'
        );
        ...
        let axia =
            (oromisthio_argion + oromisthio_argion * timhPercent) * oresYperergasiasArgion || 0;
    ...
    async function calcAxiaYperergasionArgionNyxtas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΥΠΕΡΕΡΓΑΣΙΩΝ ΑΡΓΙΩΝ ΝΥΧΤΑΣ
        if (hasRecord) return;

        let oresYperergasiasArgionNyxtas = parseFloat(
            document.getElementById('oresYperergasiasArgionNyxtas').value
        );
        let axiaYperergasiasArgionNyxtas = document.getElementById('axiaYperergasiasArgionNyxtas');
        let asfalistikhAxiaYperergasiasArgionNyxtas = document.getElementById(
            'asfalistikhAxiaYperergasiasArgionNyxtas'
        );
        ...
        let axia =
            (oromisthio_argion + oromisthio_argion * timhPercent) * oresYperergasiasArgionNyxtas ||
            0;
    ...
    async function calcAxiaNomimonYperorion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΝΟΜΙΜΩΝ ΥΠΕΡΩΡΙΩΝ
        if (hasRecord) return;

        let oresNomimhsYperorias = parseFloat(
            document.getElementById('oresNomimhsYperorias').value
        );
        let axiaNomimhsYperorias = document.getElementById('axiaNomimhsYperorias');
        let asfalistikhAxiaNomimhsYperorias = document.getElementById(
            'asfalistikhAxiaNomimhsYperorias'
        );
        ...
        if (
            document.getElementById('orioYperorion_Hidden').value >
            parseFloat(sharedParams.genikesParametroi[13].timh)
        ) {
            message = `
    ...
    async function calcAxiaNomimonYperorionArgion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΝΟΜΙΜΩΝ ΥΠΕΡΩΡΙΩΝ ΑΡΓΙΩΝ
        if (hasRecord) return;

        let oresNomimhsYperoriasArgion = parseFloat(
            document.getElementById('oresNomimhsYperoriasArgion').value
        );
        let axiaNomimhsYperoriasArgion = document.getElementById('axiaNomimhsYperoriasArgion');
        let asfalistikhAxiaNomimhsYperoriasArgion = document.getElementById(
            'asfalistikhAxiaNomimhsYperoriasArgion'
        );
        ...
        let oromisthio_argion = oromisthio + prosayxhsh_argion;
        let axia =
            (oromisthio_argion + oromisthio_argion * timhPercent) * oresNomimhsYperoriasArgion || 0;
    ...
    async function calcAxiaNomimonYperorionNyxtas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΝΟΜΙΜΩΝ ΥΠΕΡΩΡΙΩΝ ΝΥΧΤΑΣ
        if (hasRecord) return;

        let oresNomimhsYperoriasNyxtas = parseFloat(
            document.getElementById('oresNomimhsYperoriasNyxtas').value
        );
        let axiaNomimhsYperoriasNyxtas = document.getElementById('axiaNomimhsYperoriasNyxtas');
        let asfalistikhAxiaNomimhsYperoriasNyxtas = document.getElementById(
            'asfalistikhAxiaNomimhsYperoriasNyxtas'
        );
        ...
        let oromisthio_nyxtas = oromisthio + prosayxhsh_nyxtas;
        let axia =
            (oromisthio_nyxtas + oromisthio_nyxtas * timhPercent) * oresNomimhsYperoriasNyxtas || 0;
        let axia1 = oromisthio * oresNomimhsYperoriasArgion || 0;
        let prosayxhsh =
            sharedParams.ergazomenoi.plhrhs_apasxolhsh && sharedParams.ergazomenoi.karta_ergasias
                ? axia - axia1 || 0
    ...
    async function calcAxiaNomimonYperorionArgionNyxtas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΝΟΜΙΜΩΝ ΥΠΕΡΩΡΙΩΝ ΑΡΓΙΩΝ ΝΥΧΤΑΣ
        if (hasRecord) return;

        let oresNomimhsYperoriasArgionNyxtas = parseFloat(
            document.getElementById('oresNomimhsYperoriasArgionNyxtas').value
        );
        let axiaNomimhsYperoriasArgionNyxtas = document.getElementById(
            'axiaNomimhsYperoriasArgionNyxtas'
        );
        let asfalistikhAxiaNomimhsYperoriasArgionNyxtas = document.getElementById(
            'asfalistikhAxiaNomimhsYperoriasArgionNyxtas'
        );
        ...
        let oromisthio_argion = oromisthio + prosayxhsh_argion + prosayxhsh_nyxtas;
        let axia =
            (oromisthio_argion + oromisthio_argion * timhPercent) *
                oresNomimhsYperoriasArgionNyxtas || 0;
    ...
    async function calcAxiaParanomonYperorion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΠΑΡΑΝΟΜΩΝ ΥΠΕΡΩΡΙΩΝ
        if (hasRecord) return;

        let oresParanomhsYperorias = parseFloat(
            document.getElementById('oresParanomhsYperorias').value
        );
        let axiaParanomhsYperorias = document.getElementById('axiaParanomhsYperorias');
        let asfalistikhAxiaParanomhsYperorias = document.getElementById(
            'asfalistikhAxiaParanomhsYperorias'
        );
    ...
    async function calcAxiaParanomonYperorionArgion() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΠΑΡΑΝΟΜΩΝ ΥΠΕΡΩΡΙΩΝ ΑΡΓΙΩΝ
        let oresParanomhsYperoriasArgion = parseFloat(
            document.getElementById('oresParanomhsYperoriasArgion').value
        );
        let axiaParanomhsYperoriasArgion = document.getElementById('axiaParanomhsYperoriasArgion');
        let asfalistikhAxiaParanomhsYperoriasArgion = document.getElementById(
            'asfalistikhAxiaParanomhsYperoriasArgion'
        );
        ...
        let oromisthio_argion = oromisthio + prosayxhsh_argion;
        let axia =
            (oromisthio_argion + oromisthio_argion * timhPercent) * oresParanomhsYperoriasArgion ||
            0;
    ...
    async function calcAxiaParanomonYperorionNyxtas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΠΑΡΑΝΟΜΩΝ ΥΠΕΡΩΡΙΩΝ ΝΥΧΤΑΣ
        if (hasRecord) return;

        let oresParanomhsYperoriasNyxtas = parseFloat(
            document.getElementById('oresParanomhsYperoriasNyxtas').value
        );
        let axiaParanomhsYperoriasNyxtas = document.getElementById('axiaParanomhsYperoriasNyxtas');
        let asfalistikhAxiaParanomhsYperoriasNyxtas = document.getElementById(
            'asfalistikhAxiaParanomhsYperoriasNyxtas'
        );
        ...
        let oromisthio_nyxtas = oromisthio + prosayxhsh_nyxtas;
        let axia =
            (oromisthio_nyxtas + oromisthio_nyxtas * timhPercent) * oresParanomhsYperoriasNyxtas ||
            0;
    ...
    async function calcAxiaParanomonYperorionArgionNyxtas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΝΟΜΙΜΩΝ ΥΠΕΡΩΡΙΩΝ ΑΡΓΙΩΝ ΝΥΧΤΑΣ
        if (hasRecord) return;

        let oresParanomhsYperoriasArgionNyxtas = parseFloat(
            document.getElementById('oresParanomhsYperoriasArgionNyxtas').value
        );
        let axiaParanomhsYperoriasArgionNyxtas = document.getElementById(
            'axiaParanomhsYperoriasArgionNyxtas'
        );
        let asfalistikhAxiaParanomhsYperoriasArgionNyxtas = document.getElementById(
            'asfalistikhAxiaParanomhsYperoriasArgionNyxtas'
        );
        ...
        let axia =
            (oromisthio_argion + oromisthio_argion * timhPercent) *
                oresParanomhsYperoriasArgionNyxtas || 0;
    ...
    async function calcAxiaErgasias6Hmeras() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΕΡΓΑΣΙΑΣ 6ης ΗΜΕΡΑΣ
        if (hasRecord) return;
    ...
    async function calcAxiaProsthethsErgasias() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΠΡΟΣΘΕΤΗΣ ΕΡΓΑΣΙΑΣ ΜΕΡΙΚΩΣ ΑΠΑΣΧΟΛΟΥΜΕΝΩΝ
        if (hasRecord) return;

        let oresProsthethsErgasias = parseFloat(
            document.getElementById('oresProsthethsErgasias').value
        );
    ...
    async function calcAxiaMeioshsErgatikhsEisforas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΜΕΙΩΣΗΣ ΕΡΓΑΤΙΚΗΣ ΕΙΣΦΟΡΑΣ
        if (hasRecord) return;
    ...
    async function taktikesApodoxesMhYpologizomenesSeDora() {
        // ΤΑΚΤΙΚΕΣ ΑΠΟΔΟΧΕΣ ΜΗ ΥΠΟΛΟΓΙΖΟΜΕΝΕΣ ΣΕ ΔΩΡΑ
        if (hasRecord) return;

        await finishValueCalculation();
    }

    async function sympsifisteesApodoxes() {
        // ΤΑΚΤΙΚΕΣ ΑΠΟΔΟΧΕΣ ΜΗ ΥΠΟΛΟΓΙΖΟΜΕΝΕΣ ΣΕ ΔΩΡΑ
        if (hasRecord) return;
    ...
    async function taktikesApodoxesYpologizomenesSeDora() {
        // ΤΑΚΤΙΚΕΣ ΑΠΟΔΟΧΕΣ ΥΠΟΛΟΓΙΖΟΜΕΝΕΣ ΣΕ ΔΩΡΑ
        if (hasRecord) return;

        await finishValueCalculation();
    }

    async function epimerizomenesSeMhnesErgasias() {
        // ΕΚΤΑΚΤΕΣ ΑΠΟΔΟΧΕΣ ΕΠΙΜΕΡΙΖΟΜΕΝΣ ΣΕ ΣΤΟΥΣ ΜΗΝΕΣ ΕΡΓΑΣΙΑΣ
        if (hasRecord) return;

        await finishValueCalculation();
    }

    async function primBonus() {
        // ΕΚΤΑΚΤΑ ΠΡΙΜ / BONUS
        if (hasRecord) return;
    ...
    async function apallassomenesForoy() {
        // ΕΚΤΑΚΤΕΣ ΑΠΟΔΟΧΕΣ ΑΠΑΛΑΣΣΟΜΕΝΕΣ ΤΟΥ ΦΟΡΟΥ
        if (hasRecord) return;
    ...
    async function apallassomenesKrathseon() {
        // ΕΚΤΑΚΤΕΣ ΑΠΟΔΟΧΕΣ ΑΠΑΛΑΣΣΟΜΕΝΕΣ ΤΩΝ ΚΡΑΤΗΣΕΩΝ
        if (hasRecord) return;

        await finishValueCalculation();
    }

    async function synoloForoy() {
        // ΑΛΛΑΓΗ ΥΠΟΛΟΓΙΣΜΕΝΟΥ ΠΟΣΟΥ ΦΟΡΟΥ
        if (hasRecord) return;
    ...
    async function validateFields() {
        // ΕΛΕΓΧΟΣ ΑΝ ΕΧΕΙΣ ΔΟΘΕΙ ΠΟΣΟ ΠΡΩΤΑ ΣΤΙΣ ΑΠΑΛΛΑΣΣΟΜΕΝΕΣ ΤΩΝ ΚΡΑΤΗΣΕΩΝ ΚΑΙ ΜΕΤΑ ΤΟΥ ΦΟΡΟΥ
    ...
    exoflhsh.addEventListener('change', async () => {
        // EΝΗΜΕΡΩΣΗ ΤΟΥ ΠΟΣΟΥ ΠΛΗΡΩΜΗΣ
        if (hasRecord) return;

        let selectedValue = exoflhsh.value;
        switch (selectedValue) {
            case '0':
                document.getElementById('poso_plhromhs').value =
                    parseFloat(document.getElementById('plhroteo').value) || 0;
                document.getElementById('exoflhsh_Hidden').value = '0';
                document.getElementById('poso_plhromhs_Hidden').value =
                    parseFloat(document.getElementById('plhroteo').value) || 0;
    ...
    posoPlhromhs.addEventListener('blur', async () => {
        document.getElementById('poso_plhromhs_Hidden').value = parseFloat(
            document.getElementById('poso_plhromhs').value
        );
    });

    document.getElementById('meioshErgatikhsEisforas').addEventListener('input', (event) => {
        // ΜΕΙΩΣΗ ΕΡΓΑΤΙΚΗΣ ΕΙΣΦΟΡΑΣ
        if (hasRecord) return;
    ...
    async function calcAxiaEpidothshsErgodotikhsEisforas() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΑΞΙΑΣ ΕΠΙΔΟΤΗΣΗΣ ΕΡΓΟΔΟΤΙΚΗΣ ΕΙΣΦΟΡΑΣ (ΕΡΓΑΖΟΜΕΝΟΙ < 25 ΕΤΩΝ)
        if (hasRecord) return;

        if (
            !sharedParams.ergazomenoi.hmeromhnia_gennhshs ||
            !sharedParams.ergazomenoi.epidothsh_eisforon_ergodoth
        ) {
        ...
        if (age && age < parseFloat(sharedParams.genikesParametroi[19].timh)) {
            ...
            document.getElementById('epidothshErgodotikhsEisforas').value = (
                parseFloat(document.getElementById('synoloMiktonApodoxon').value) * timhPercent
            ).toFixed(2);
        }
    }
    ...
    async function calcSynoloAsfalistikonApodoxon() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΣΥΝΟΛΟΥ ΑΣΦΑΛΙΣΤΙΚΩΝ ΑΠΟΔΟΧΩΝ
        if (hasRecord) return;
    ...
    async function calcSynoloMiktonApodoxonCore() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΣΥΝΟΛΟΥ ΜΙΚΤΩΝ ΑΠΟΔΟΧΩΝ
        if (hasRecord) return;
        ...
        prosayxhseis = parseFloat(
            parseFloat(document.getElementById('axiaArgion').value || 0) +
                ...
                parseFloat(
                    document.getElementById('axiaParanomhsYperoriasArgionNyxtas').value || 0
                ) +
        ...
        const synoloApodoxonBase = safeNumber(
        ...
        synolo_mikton_apodoxon =
            ...
            safeNumber(document.getElementById('sympsifisteesApodoxes')?.value) +
            safeNumber(document.getElementById('epimerizomenesSeMhnesErgasias')?.value) +
            safeNumber(document.getElementById('primBonus')?.value) +
            safeNumber(document.getElementById('apallassomenesForoy')?.value) +
            safeNumber(document.getElementById('apallassomenesKrathseon')?.value) -
            safeNumber(document.getElementById('geniko_synolo_astheneias')?.value);
        ...
        document.getElementById('neoPragmatikoHmeromisthio').value =
            parseFloat(document.getElementById('hmeresAsfalishs').value) === 0
                ? parseFloat(synolo_mikton_apodoxon)
                : (
                      parseFloat(
                          parseFloat(synolo_mikton_apodoxon) /
                              parseFloat(document.getElementById('hmeresAsfalishs').value)
                      ) || 0
                  ).toFixed(4);

        document.getElementById('synoloTaktikaKataballomenonApodoxon').value = parseFloat(
            parseFloat(
                document.getElementById('taktikesApodoxesMhYpologizomenesSeDora').value || 0
            ) +
                parseFloat(
                    document.getElementById('taktikesApodoxesYpologizomenesSeDora').value || 0
                ) +
    ...
    async function calcPlhroteoCore() {
        // ΥΠΟΛΟΓΙΣΜΟΣ ΠΛΗΡΩΤΕΟΥ ΠΟΣΟΥ
        if (hasRecord) return;
        ...
        const epimerizomenesSeMhnesErgasias = document.getElementById(
            'epimerizomenesSeMhnesErgasias'
        ).value;
        const floatValue_EpimerizomenesSeMhnesErgasias = safeNumber(epimerizomenesSeMhnesErgasias);
        const synolo_axias_krathshs_ergazomenoy_ypologizomenh_sto_foro = document.getElementById(
            'synolo_axias_krathshs_ergazomenoy_ypologizomenh_sto_foro'
        ).value;
        const floatValue_synolo_axias_krathshs_ergazomenoy_ypologizomenh_sto_foro = safeNumber(
            synolo_axias_krathshs_ergazomenoy_ypologizomenh_sto_foro
        );
        ...
        const forologhteo_poso_taktikon_apodoxon = document.getElementById(
            'forologhteo_poso_taktikon_apodoxon'
        );
        ...
        ethsio_forologhteo_eisodhma = parseFloat(
            forologhteo_poso_taktikon_apodoxon.value * _MHNES_YPOLOGISMOY_FOROY
        ).toFixed(2);
        document.getElementById('ethsio_forologhteo_poso_taktikon_apodoxon').value =
            ethsio_forologhteo_eisodhma;
        ...
        if (result) {
            ...
            document.getElementById('synoloForoy').value =
                parseFloat(floatValue_SynoloForoy).toFixed(2);

            plhroteo.value = (
                parseFloat(document.getElementById('synoloMiktonApodoxon').value || 0) +
                parseFloat(document.getElementById('meioshErgatikhsEisforas').value || 0) -
                floatValue -
                floatValue_SynoloForoy
            ).toFixed(2);
    ...
    const fields = [
        {
            id: 'oresErgasias',
            functions: { calc: calcOresErgasias, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'hmeresApoysias',
            functions: {
                calc: calcHmeresErgasiasMeionApoysies,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresApoysias',
            functions: { calc: calcOresApoysias, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'oresArgion',
            functions: { calc: calcAxiaArgion, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'oresNyxtas',
            functions: { calc: calcAxiaNyxterinon, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'oresYperergasias',
            functions: { calc: calcAxiaYperergasion, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'oresYperergasiasArgion',
            functions: {
                calc: calcAxiaYperergasionArgion,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresYperergasiasNyxtas',
            functions: {
                calc: calcAxiaYperergasionNyxtas,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresYperergasiasArgionNyxtas',
            functions: {
                calc: calcAxiaYperergasionArgionNyxtas,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresNomimhsYperorias',
            functions: {
                calc: calcAxiaNomimonYperorion,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresNomimhsYperoriasArgion',
            functions: {
                calc: calcAxiaNomimonYperorionArgion,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresNomimhsYperoriasNyxtas',
            functions: {
                calc: calcAxiaNomimonYperorionNyxtas,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresNomimhsYperoriasArgionNyxtas',
            functions: {
                calc: calcAxiaNomimonYperorionArgionNyxtas,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresParanomhsYperorias',
            functions: {
                calc: calcAxiaParanomonYperorion,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresParanomhsYperoriasArgion',
            functions: {
                calc: calcAxiaParanomonYperorionArgion,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresParanomhsYperoriasNyxtas',
            functions: {
                calc: calcAxiaParanomonYperorionNyxtas,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresParanomhsYperoriasArgionNyxtas',
            functions: {
                calc: calcAxiaParanomonYperorionArgionNyxtas,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresErgasias6Hmeras',
            functions: {
                calc: calcAxiaErgasias6Hmeras,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'oresProsthethsErgasias',
            functions: {
                calc: calcAxiaProsthethsErgasias,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'taktikesApodoxesMhYpologizomenesSeDora',
            functions: {
                calc: taktikesApodoxesMhYpologizomenesSeDora,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'taktikesApodoxesYpologizomenesSeDora',
            functions: {
                calc: taktikesApodoxesYpologizomenesSeDora,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'sympsifisteesApodoxes',
            functions: { calc: sympsifisteesApodoxes, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'epimerizomenesSeMhnesErgasias',
            functions: {
                calc: epimerizomenesSeMhnesErgasias,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'primBonus',
            functions: { calc: primBonus, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'apallassomenesForoy',
            functions: { calc: apallassomenesForoy, blur: (event) => formatInputOnBlur(event, 2) }
        },
        {
            id: 'apallassomenesKrathseon',
            functions: {
                calc: apallassomenesKrathseon,
                blur: (event) => formatInputOnBlur(event, 2)
            }
        },
        {
            id: 'synoloForoy',
            functions: { calc: synoloForoy, blur: (event) => formatInputOnBlur(event, 2) }
        },
            ...
            functions: {
                ...
                dblclick: (event) => {
                    ...
                    if (_SYNOLO_MIKTON_APODOXON === null) {
                        _SYNOLO_MIKTON_APODOXON = parseFloat(
                            document.getElementById('synoloMiktonApodoxon').value
                        );
                    }
                    disableKeyupForPlhroteo = true; // Απενεργοποίηση του keyup
                    event.target.removeAttribute('readonly');
                    document.getElementById('plhroteo').value =
                        document.getElementById('symfonhtheisMisthos').value;
                    event.target.focus();
                }
            }
        }
    ...
    async function ypologismosAxiasKrathseon(sharedParams) {
        ...
        const synoloAxiasFields = {
            ...
            ergazomenoyYpologizomenhStoForo: document.getElementById(
                'synolo_axias_krathshs_ergazomenoy_ypologizomenh_sto_foro'
            ),
            ergazomenoyMhYpologizomenhStoForo: document.getElementById(
                'synolo_axias_krathshs_ergazomenoy_mh_ypologizomenh_sto_foro'
            ),
            ergodothYpologizomenhStoForo: document.getElementById(
                'synolo_axias_krathshs_ergodoth_ypologizomenh_sto_foro'
            ),
            ergodothMhYpologizomenhStoForo: document.getElementById(
        ...
        function getAnotatoOrio(index) {
            if (
                !sharedParams?.ergazomenoi ||
                typeof sharedParams.ergazomenoi.palios_neos === 'undefined'
            ) {
            ...
            const fieldId = isNeos ? `anotato_orio_neon_${index}` : `anotato_orio_palion_${index}`;
        ...
        async function initializeInitialValues() {
            for (let i = 1; i <= 7; i++) {
                ...
                const epiPlasmatikhsEl = document.getElementById(
                    `ypologizomenoEpiPlasmatikhs_${idx}`
                );
                ...
                if (epiPlasmatikhsEl && plasmatikhAxiaEl) {
                    ...
                    const plasmatikhVal = parseFloat(
                        (plasmatikhAxiaEl.value || '0').replace(',', '.')
                    );
        ...
        async function calculateValues() {
            ...
            for (let i = 1; i <= 7; i++) {
                ...
                const fields = {
                    ...
                    posostoErgazomenoy: document.getElementById(
                        `pososto_krathshs_ergazomenoy_${idx}`
                    ),
                ...
                let asfVal = parseFloat(
                    (fields.asfalistikesApodoxes.value || '0').replace(',', '.')
                );
                ...
                const pErgaz =
                    parseFloat((fields.posostoErgazomenoy.value || '0').replace(',', '.')) / 100;
                const pErgod =
                    parseFloat((fields.posostoErgodoth.value || '0').replace(',', '.')) / 100;
            ...
            // Ενημέρωση συνόλων κρατήσεων.
            // Τα αντίστοιχα πεδία είναι <input type="number">, άρα η τιμή πρέπει
    ...
    async function updatePlhroteoWithComparison() {
        ...
        if (!isNaN(valueAllagh) && valueAllagh !== 0) {
            ...
            if (diafora !== 0) {
                ...
                let floatValue_SynoloForoy = parseFloat(
                    synoloForoyField.value.replace(',', '.') || '0.00'
                );
    ...
    async function calcNeesSympsifisteesApodoxes() {
        ...
        document.getElementById('plhroteo').value = parseFloat(
            parseFloat(_NEES_PLHROTEES_APODOXES) -
                parseFloat(document.getElementById('prokatabolh').value)
        ).toFixed(2);
        ...
        document.getElementById('sympsifisteesApodoxes').value = parseFloat(
            parseFloat(document.getElementById('synoloMiktonApodoxon').value) -
                parseFloat(_PROHGOYMENES_MIKTES_APODOXES)
        ).toFixed(2);
    }
});
