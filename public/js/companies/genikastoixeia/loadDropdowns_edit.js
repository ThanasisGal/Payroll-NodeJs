...
async function loadNomikesMorfesOptions() {
  try {
    // Fetch τα δεδομένα DOY
    const nomikesMorfesResponse = await fetch('/api/nomikesMorfes');
    const nomikesMorfesData = await nomikesMorfesResponse.json();

    // Fetch τα δεδομένα της εταιρείας
    const companyId = document.getElementById("companyId").value;
    const companyResponse = await fetch(`/api/companies/${companyId}`);
    const companyData = await companyResponse.json();
    fillSelectElement('nomikhmorfh', nomikesMorfesData, companyData.nomikh_morfh);

    // Fetch τα δεδομένα του έμμεσου εργοδότη
    if (companyData.emmesos_ergodoths === "") {
      fillSelectElement('nomikh_morfh_em_erg', nomikesMorfesData, null); // Παράδειγμα κλήσης με null ως selectedValue
    } else {
      const emmesosErgodothsResponse = await fetch(`/api/emmesosErgodoths/${companyData.emmesos_ergodoths}`);
      const emmesosErgodothsData = await emmesosErgodothsResponse.json();
      // Έλεγχος αν το πεδίο emmesos_ergodoths υπάρχει στη Β.Δ. ή έχει προστεθεί εκ των υστέρων
      if (emmesosErgodothsData && emmesosErgodothsData.length > 0 && emmesosErgodothsData[0].hasOwnProperty('emmesos_ergodoths')) {
        fillSelectElement('nomikh_morfh_em_erg', nomikesMorfesData, emmesosErgodothsData[0].nomikhMorfh);
      } else {
        if (emmesosErgodothsData && Array.isArray(emmesosErgodothsData) && emmesosErgodothsData.length > 0) {
          // Εδώ ο κώδικας υποθέτει ότι η μορφή δεδομένων είναι σωστή και προχωράει στην επεξεργασία
          fillSelectElement('nomikh_morfh_em_erg', nomikesMorfesData, emmesosErgodothsData[0].nomikhMorfh);
        }
      }
    }    

    // Fetch τα δεδομένα του διάδοχου εργοδότη
    if (companyData.diadoxos_ergodoths === "") {
      fillSelectElement('nomikh_morfh_diad_erg', nomikesMorfesData, null); // Παράδειγμα κλήσης με null ως selectedValue
    } else {
      const diadoxosErgodothsResponse = await fetch(`/api/diadoxosErgodoths/${companyData.diadoxos_ergodoths}`);
      const diadoxosErgodothsData = await diadoxosErgodothsResponse.json();
      // Έλεγχος αν το πεδίο diadoxos_ergodoths υπάρχει στη Β.Δ. ή έχει προστεθεί εκ των υστέρων
      if (diadoxosErgodothsData && diadoxosErgodothsData.length > 0 && diadoxosErgodothsData[0].hasOwnProperty('emmesos_ergodoths')) {
        fillSelectElement('nomikh_morfh_em_erg', nomikesMorfesData, diadoxosErgodothsData[0].nomikhMorfh);
      } else {
        if (diadoxosErgodothsData && Array.isArray(diadoxosErgodothsData) && diadoxosErgodothsData.length > 0) {
          // Εδώ ο κώδικας υποθέτει ότι η μορφή δεδομένων είναι σωστή και προχωράει στην επεξεργασία
          fillSelectElement('nomikh_morfh_em_erg', nomikesMorfesData, diadoxosErgodothsData[0].nomikhMorfh);
        }
      }
