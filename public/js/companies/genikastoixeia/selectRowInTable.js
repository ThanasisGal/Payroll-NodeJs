    ...
    rows.forEach(function (row) {
        row.addEventListener("click", function () {
            if (this.classList.contains("selected-row")) {
                ...
                if (selectedRowId) {
                    document.getElementById("select-btn").href = "/companies/genikastoixeia/select/" + selectedRowId;
                    document.getElementById("edit-btn").href = "/companies/genikastoixeia/edit/" + selectedRowId;
                    document.getElementById("delete-btn").addEventListener("click", async function (event) {
                        // Ορίζει το URL για διαγραφή
                        var deleteUrl = "/companies/genikastoixeia/delete/" + selectedRowId;

                        const result = await Swal.fire({
                            title: "Είστε σίγουρος / η;",
                            html: `ΠΡΟΣΟΧΗ!!! <br>Δεν θα μπορείτε να αναιρέσετε αυτή την ενέργεια! Με την διαγραφή της εταιρείας θα διαγραφούν (απ΄ όλα τα αρχεία) οι εγγραφές, που αφορούν την συγκεκριμένη εταιρεία.`,
                            icon: "error",
