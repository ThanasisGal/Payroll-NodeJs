document.addEventListener("DOMContentLoaded", () => {
    async function handleFormSubmit(event) {
        ...
        try {
            ...
            const trapezesId = document.getElementById("trapezesId").value;
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content || "";

            const response = await fetch(`/api/trapezes/update/${trapezesId}`, {
                method: "POST",
                headers: {
            ...
            if (response.status === 204) {
                ...
                }).then(() => window.location.href = `/companies/trapezes/edit/${trapezesId}`);
                return;
            }
            ...
            if (ct.includes("application/json")) {
                ...
                }).then(() => window.location.href = data.redirectUrl || `/companies/trapezes/edit/${trapezesId}`);
                return;
            }
