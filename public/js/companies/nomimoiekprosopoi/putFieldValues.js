document.addEventListener("DOMContentLoaded", () => {
    async function handleFormSubmit(event) {
        ...
        try {
            ...
            const nomimosEkprosoposId = document.getElementById("nomimosEkprosoposId").value;
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content || "";

            const response = await fetch(`/api/nomimoi_ekprosopoi/update/${nomimosEkprosoposId}`, {
                method: "POST",
                headers: {
            ...
            if (response.status === 204) {
                ...
                }).then(() => window.location.href = `/companies/nomimoi_ekprosopoi/edit/${nomimosEkprosoposId}`);
                return;
            }
            ...
            if (ct.includes("application/json")) {
                ...
                }).then(() => window.location.href = data.redirectUrl || `/companies/nomimoi_ekprosopoi/edit/${nomimosEkprosoposId}`);
                return;
            }
