document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("myTable").addEventListener("click", function (event) {
    if (event.target.closest('.btn-nesting')) return;

    let targetRow = event.target.closest("tr[data-id]");
    if (!targetRow) return;

    const isSelected = targetRow.classList.contains("selected-row");
    document.querySelectorAll("#myTable .selected-row").forEach(row => row.classList.remove("selected-row"));
    ...
    if (!isSelected) {
      ...
      document.getElementById("edit-btn").innerHTML = `<i class="buttons-content bi bi-pencil-square"></i> Συντήρηση Κράτησης`;
      adjustFontSizeToFit("edit-btn", 15); // Προσαρμόζει το μήκος του κειμένου στο πλάτος του button
      document.getElementById("delete-btn").innerHTML = `<i class="buttons-content bi bi-trash"></i> Διαγραφή Κράτησης`;  
