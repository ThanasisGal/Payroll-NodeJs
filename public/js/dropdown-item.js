...
// // const DEBUG_TOM = true;
// // const DEBUG_TOM_STORAGE_KEY = '__tom_debug_logs__';
// // const DEBUG_TOM_STORAGE_LIMIT = 200; // ← από 1000 σε 200 για performance

// // function dbg(id, phase, data = {}) {
// //     if (!DEBUG_TOM) return;
// //     try {
// //         const now = new Date().toISOString().slice(11, 23);
// //         const entry = { ts: now, id: id || '(unknown)', phase, data: data || {} };
// //         console.log(`[TOMDBG ${now}] ${entry.id} :: ${phase}`, entry.data);

// //         try {
// //             const arr = JSON.parse(localStorage.getItem(DEBUG_TOM_STORAGE_KEY) || '[]');
// //             arr.push(entry);
// //             if (arr.length > DEBUG_TOM_STORAGE_LIMIT) {
// //                 arr.splice(0, arr.length - DEBUG_TOM_STORAGE_LIMIT);
// //             }
// //             localStorage.setItem(DEBUG_TOM_STORAGE_KEY, JSON.stringify(arr));
// //         } catch (_) {}
// //     } catch (_) {}
// // }

// // function dbgCount(obj, key) {
// //     try {
// //         obj[key] = (obj[key] || 0) + 1;
// //         return obj[key];
// //     } catch (_) {
...
// // window.__getTomDebugLogs = function () {
// //     try {
// //         return JSON.parse(localStorage.getItem(DEBUG_TOM_STORAGE_KEY) || '[]');
// //     } catch (_) {
// //         return [];
// //     }
// // };

// // window.__clearTomDebugLogs = function () {
// //     try {
// //         localStorage.removeItem(DEBUG_TOM_STORAGE_KEY);
// //     } catch (_) {}
// // };
...
//     // ✅ FIX #3 (MutationObserver): debounced για να μην κάνει loop
...
//     // dbg(el.id || selector || '(unknown)', 'initTomDropdown:start', {
//     //     selector,
//     //     url,
//     //     isMultiple,
//     //     preloadAll,
//     //     padLength: el.dataset.padLength || null,
//     //     preselect: el.dataset.preselect || null,
//     //     targetInput: el.dataset.targetInput || null,
//     //     targetTable: el.dataset.targetTable || null
//     // });
...
//             // dbg(el.id || selector || '(unknown)', 'load:enter', {
//             //     searchText,
//             //     lastQuery: this.lastQuery,
//             //     itemsCount: Array.isArray(this.items) ? this.items.length : null,
//             //     optionsCount: this.options ? Object.keys(this.options).length : null,
//             //     count: dbgCount(this, '__dbg_load_count')
//             // });
...
//                 // dbg(el.id || selector || '(unknown)', 'load:urlBuilt', { urlToFetch });
//             }
...
//                 // dbg(el.id || selector || '(unknown)', 'load:fetched', {
//                 //     urlToFetch,
//                 //     rawCount: items.length,
//                 //     hasMoreFlag: Object.prototype.hasOwnProperty.call(json || {}, 'hasMore')
//                 //         ? json.hasMore
//                 //         : '(derived)'
//                 // });
...
//                 // dbg(el.id || selector || '(unknown)', 'load:normalized', {
//                 //     normalizedCount: items.length,
//                 //     selectedItems: Array.isArray(this.items) ? [...this.items] : null
//                 // });
...
//                 // dbg(el.id || selector || '(unknown)', 'load:done', {
...
//             // console.log('PATCH_CHECK', el.id, { isMultiple });

//             // dbg(el.id || selector || '(unknown)', 'onInitialize', {
//             //     mode: this.settings.mode,
//             //     isMultiple,
//             //     preloadAll: this.settings._preloadAll
//             // });
...
//             // console.log('PATCH_MULTIPLE_ONLY_BLOCK', el.id, { isMultiple });
...
//                         // ✅ Disable μετά από επιλογή (single mode)
//                         this.disable();
//                         this.wrapper.classList.add('ts-disabled-selected');
//                         this.wrapper.classList.add('ts-locked');

//                         // ✅ Chain callback — καλεί οποιοδήποτε hook έχει οριστεί εξωτερικά
...
//                 // ✅ ΝΕΟ: κάλεσε το chain hook αν υπάρχει
...
//             // ✅ FIX #2: guard στο dropdown_open
//             this.on('dropdown_open', () => {
//                 if (this._preselecting || this._suspendItemAddFlow) return;

//                 // dbg(el.id || selector || '(unknown)', 'event:dropdown_open', {
//                 //     optionsCount: this.options ? Object.keys(this.options).length : null,
//                 //     items: Array.isArray(this.items) ? [...this.items] : null,
//                 //     textboxValue: this.control_input?.value || '',
//                 //     count: dbgCount(this, '__dbg_dropdown_open_count')
//                 // });
...
//                 // ✅ FIX #4 (overflowNow): guard κατά preselect
//                 const overflowNow = () => {
//                     if (this._preselecting || this._suspendItemAddFlow) return;
...
//                 // ✅ FIX #4 (refresh): guard κατά preselect
...
//             } else {
//                 // Single: updateOverflow για trash button
...
//                 // dbg(el.id || selector || '(unknown)', 'doPreselect:start', {
//                 //     preselectId: el.dataset.preselect || null,
//                 //     targetInput: el.dataset.targetInput || null,
//                 //     targetTable: el.dataset.targetTable || null
//                 // });
...
//                 // dbg(el.id || selector || '(unknown)', 'doPreselect:raw', { preSelRaw });
...
//                     // dbg(el.id || selector || '(unknown)', 'doPreselect:multiple:values', {
//                     //     values
//                     // });
...
//                             // dbg(el.id || selector || '(unknown)', 'doPreselect:multiple:fetch', {
//                             //     val,
//                             //     url: u.toString()
//                             // });
...
//                                 // dbg(
//                                 //     el.id || selector || '(unknown)',
//                                 //     'doPreselect:multiple:addItem',
//                                 //     { val, id, label: normalized.label }
//                                 // );
...
//                     // dbg(el.id || selector || '(unknown)', 'doPreselect:multiple:done', {
//                     //     items: Array.isArray(ts.items) ? [...ts.items] : null
//                     // });
...
//                         updateOverflow(ts);
//                         // ✅ Disable μετά από preselect (single mode)
//                         ts.disable();
//                         ts.wrapper.classList.add('ts-disabled-selected');
//                         ts.wrapper.classList.add('ts-locked');
//                     });

//                     // Ενημέρωσε οποιοδήποτε data-link-target έχει οριστεί
...
//                     // ✅ FIX #8: guard στο writeTable
//                     const writeTable = () => {
//                         if (this._preselecting || this._suspendItemAddFlow) return;

//                         // dbg(el.id || selector || '(unknown)', 'writeTable:enter', {
//                         //     values: this.getValue(),
//                         //     count: dbgCount(this, '__dbg_writeTable_count')
//                         // });
...
//                                 // dbg(el.id || selector || '(unknown)', 'writeTable:update', {
//                                 //     nextValue,
...
//         // ✅ FIX #2: guard στο onDropdownOpen
//         onDropdownOpen() {
//             if (this._preselecting || this._suspendItemAddFlow) return;
...
//             // ΔΕΝ καλούμε addOption — το option ήδη υπάρχει στο tom.options
//         } else {
...
//     // dbg(tom?.input?.id || '(unknown)', 'updateOverflow:enter', {
//     //     mode: tom?.settings?.mode,
//     //     items: Array.isArray(tom?.items) ? [...tom.items] : null,
//     //     count: dbgCount(tom || {}, '__dbg_updateOverflow_count')
//     // });
...
//                 // ✅ Re-enable πριν το clear
//                 tom.enable();
//                 tom.wrapper.classList.remove('ts-disabled-selected');
//                 tom.wrapper.classList.remove('ts-locked'); // ✅ ΝΕΟ
...
//         // ✅ FIX #6: forceChange χωρίς διπλό trigger
...
//             // ✅ FIX #1 (updateResetIcon): guard κατά preselect
