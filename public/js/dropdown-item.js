...
export const initTomDropdown = ({
    ...
    const tom = new TomSelect(el, {
                ...
                items = items
                    ...
                    .map((it) => {
                        const rawVal =
                            ...
                            it.value ??
                            it.id ??
                            (it.kodikos != null ? String(it.kodikos).padStart(4, '0') : '');
                        ...
                        const label = String(
                            it[lKey] ??
                                ...
                                it.text ??
                                (it.kodikos != null
                                    ? `${String(it.kodikos).padStart(4, '0')} - ${it.perigrafh ?? ''}`
                                    : val)
                        );
                        return {
                            ...it,
        ...
        onInitialize() {
            ...
            /* ============================================================
             * 🆕 D.  AUTO-POPULATE HIDDEN FIELDS (από data-field-mappings)
             * ============================================================ */
                ...
                try {
                    ...
                    this.on('change', (value) => {
                        ...
                        Object.keys(mappings).forEach((sourceField) => {
                            ...
                            // ✅ ΑΛΛΑΞΕ ΕΔΩ:  Έλεγχος αν το TARGET field είναι pososto_*
                            if (targetFieldId.includes('pososto_')) {
                                fieldValue = formatDecimal(fieldValue, 3); // 3 δεκαδικά
                            } else if (
...
function updateOverflow(tom) {
    ...
    if (isMulti) {
        ...
        if (!resetBtn.__wired) {
            ...
            // Helper: ενημερώνει το εικονίδιο ανάλογα με την κατάσταση
  ...
  <path d="M8.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L2.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093L8.95 4.992zm-.92 5.14.92.92a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 1 0-1.091-1.028L9.477 9.417l-.485-.486z"/>
            ...
            const updateResetIcon = () => {
                ...
                if (hasTags) {
                    resetBtn.innerHTML = SVG_TRASH; // ✅ ίδιο SVG με single trash
                    resetBtn.title = 'Καθαρισμός όλων';
                    resetBtn.hidden = false;
                } else {
                    resetBtn.innerHTML = SVG_CHECK; // ✅ check-square SVG
                    resetBtn.title = 'Επιλογή όλων';
                    resetBtn.hidden = false; // ✅ πάντα ορατό για multi
                }
            };

            // Κάλεσέ το αμέσως για να ρυθμίσει το αρχικό state
            updateResetIcon();

            // Events — μόνο μια φορά
