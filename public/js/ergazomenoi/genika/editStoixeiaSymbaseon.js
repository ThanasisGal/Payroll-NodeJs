...
function generateSelectRowsOfSymbaseis() {
  ...
  for (let i = 1; i <= 15; i++) {
    // if (document.getElementById("stoixeioSymbashsHidden_" + i.toString().padStart(2, '0')).value !== "") {
      const selectedStoixeioSymbashsValue = document.getElementById("stoixeioSymbashsHidden_" + i.toString().padStart(2, '0')).value;
      const selectedPosoSymbashsValue = parseFloat(document.getElementById("posoSymbashsHidden_" + i.toString().padStart(2, '0')).value).toFixed(2);
      const selectedPosoSymbashsBaseiOronErgasiasValue = parseFloat(document.getElementById("posoSymbashsBaseiOronErgasiasHidden_" + i.toString().padStart(2, '0')).value).toFixed(2);
  
      const readOnlyAttribute = selectedPosoSymbashsValue !== '0.00' ? 'readonly' : '';
      ...
      const rowHTML = `
        <div class="row form-group align-items-center d-none showhide_row_${i.toString().padStart(2, '0')}" style="background-color: #ffffff; margin-left: -1rem; margin-right: -1rem; margin-bottom: -0.6rem !important;" id="row_${i.toString().padStart(2, '0')}">
          ...
          <div class="col-7">
            <select class="form-select selectpicker-dropdown-normal" data-live-search="true" name="stoixeio_symbashs_${i.toString().padStart(2, '0')}" id="stoixeio_symbashs_${i.toString().padStart(2, '0')}" data-selected="${selectedStoixeioSymbashsValue}">
            </select>
          </div>
          <div class="col-1-5">
            <input type="number" class="form-control input-contents right-align clearAble" name="poso_symbashs_${i.toString().padStart(2, '0')}" id="poso_symbashs_${i.toString().padStart(2, '0')}" style="background-color: #eeff0223;" value="${selectedPosoSymbashsValue}" readonly />
          </div>
...
async function handleInputEvent(event) {
  if (event.target.type === 'text' && event.target.classList.contains('numeric')) {
    ...
    const selectedElem = "stoixeio_symbashs_" + elementId.substring(34);
    const selectElement = document.getElementById(selectedElem).value;
    ...
    data = await fetchApodoxesData(contract, category, specialty, selectElement, klimakio, hmeromhniaAllaghsSymbashs);
    data.poso = newValue;

    const posoBasedOnHoursFieldId = elementId;
    let extraApodoxes = true;

    updateGeneralParameters(data.genikesParametroi);
    updatePosoBasedOnHours(parseInt(elementId.substring(34)), data, posoBasedOnHoursFieldId, extraApodoxes);
...
function updateGeneralParameters(parameters) {
    ...
    _ORES_EIDIKHS_KATHGORIAS_PLHROYS_APASXOLHSHS = parseFloat(parameters[6].timh);
}
...
async function calculateTotal() {
  ...
  for (let i = 1; i <= 15; i++) {
    if (document.getElementById(`poso_symbashs_${i.toString().padStart(2, '0')}`) !== null) {
  ...
  if (_PLHRHS_APASXOLHSH.checked) {
    switch (document.getElementById('typos_ergazomenon').value) {
      ...
      case "Μ":
        ...
        // pragmatikoOromisthio.value = _PRAGMATIKO_OROMISTHIO.toFixed(4);
        pragmatikosMisthos.value = (_PRAGMATIKO_OROMISTHIO * parseFloat(ores_ergasias_ebdomadas) * _SYNTELESTHS_EBDOMADON_MISTHOTON).toFixed(2);
        pragmatikoHmeromisthio.value = parseFloat(_PRAGMATIKO_OROMISTHIO * parseFloat(_ORES_HMERHSIAS_ERGASIAS)).toFixed(2);
        pragmatikoOromisthio.value = _PRAGMATIKO_OROMISTHIO.toFixed(4);
        break;
      case "Η":
    ...
    switch (document.getElementById('typos_ergazomenon').value) {
      case "Ω":
        switch (document.getElementById('eidikh_kathgoria_ergazomenoy').value) {
          case "0001": // ΚΑΘΗΓΗΤΕΣ ΦΡΟΝΤΙΣΤΗΡΙΩΝ
            _ORES_EIDIKHS_KATHGORIAS_PLHROYS_APASXOLHSHS === 0 ? document.getElementById("eidikhKathgoriaErgazomenoy").value : _ORES_EIDIKHS_KATHGORIAS_PLHROYS_APASXOLHSHS
            _NOMIMO_OROMISTHIO = (6 / _ORES_EIDIKHS_KATHGORIAS_PLHROYS_APASXOLHSHS / 25) * totalSymbashs;
