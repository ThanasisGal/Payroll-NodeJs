document.addEventListener('DOMContentLoaded', async function () {
    ...
    document.getElementById('ergazomenos').addEventListener('change', function (event) {
        ...
        fromEmployee =
            'του εργαζόμενου ' +
            selectedOption.text.substring(0, 30).trim() +
            ' ' +
            selectedOption.text.substring(30, 39).trim() +
            ' ' +
            selectedOption.text.substring(39, 59).trim();
    });
    ...
    function updateHref() {
        ...
        const daysDiff = Math.ceil(
            (new Date(fromEndDate) - new Date(fromStartDate)) / (1000 * 60 * 60 * 24)
        );
        if (daysDiff > 6) {
