...
function resolveAmkaCentury(dd, mm, yy, { minAge = 0, maxAge = 110, strategy = 'cutoff' } = {}) {
    ...
    if (strategy === 'prefer2000')
        return candidates.find((dt) => dt.getFullYear() >= 2000) || candidates[0];
    if (strategy === 'prefer1900')
        return candidates.find((dt) => dt.getFullYear() < 2000) || candidates[0];
        ...
        if (birthInput) {
            if (res && res.ok && res.birthDate) {
                ...
                if (birthInput.value !== iso) {
                    ...
                    // Focus + Blur για trigger validation
