    ...
    if (response.ok) {
        ...
        if (data) {
            ...
            if (data.doc) {
                ...
                document.getElementById("doy_logisths").value = data.doc.doy;

                /* ------------------------------------------------------------------ */
                /*  Δείξε τη Δ.Ο.Υ. στο TomSelect (#doy_accountant)                   */
                /* ------------------------------------------------------------------ */
                const selectEl = document.querySelector('#doy_accountant');
                const padLen   = Number(selectEl?.dataset.padLength) || 0;   // 4

                const ts  = window.__tomInstances?.['#doy_accountant'];
                if (ts) {
                    const val = String(data.doc.doy);

                    /* helper: φτιάχνει label με σωστό padding */
                    const buildLabel = (v, txt='') => {
                        const padded = v.padEnd(padLen, ' ');
                        const label  = `${padded} - ${txt}`;
                        return { value: v, text: label };
                    };

                    if (ts.options[val]) {
                        /* option υπάρχει: ενημέρωσε το text αν χρειάζεται */
                        ...
                        /* fetch με padLength */
                        fetch(`/api/dropdown/logistes?value=${encodeURIComponent(val)}&padLength=${padLen}`)
                        .then(r => r.json())
                        .then(({ items }) => {
                            if (items?.length) {
                                const item = items[0];
                                ...
                                /* fallback αν API δεν έστειλε label */
