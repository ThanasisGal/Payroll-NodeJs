#!/usr/bin/env bash
# =============================================================================
# Production Deployment Script v3.0 - MULTI-OS SUPPORT
# IMPROVEMENTS: 
#   - Auto-detects OS (Cygwin/Linux/macOS)
#   - Unified script for Windows (Cygwin) and Ubuntu
...
# =============================================================================
# CONFIGURATION - AUTO-ADJUSTED PER OS
# =============================================================================

# Detect home directory and project path
if [[ "$OS_TYPE" == "cygwin" ]]; then
    ...
    # Path for PowerShell DNS update (Windows only)
    DNS_UPDATE_SCRIPT="C:\\Users\\User\\Desktop\\AWS-EC2-S3\\deploy.ps1"
    HAS_POWERSHELL=true
else
    # Linux/Ubuntu or macOS
    PROJECT_DIR="$HOME/Payroll-NodeJs"
...
# OS-specific file size function
get_file_size() {
    local file="$1"
...
# OS-specific path conversion (only for Cygwin)
normalize_path() {
    local path="$1"
    if [[ "$OS_TYPE" == "cygwin" ]]; then
...
if [[ !  -d "$PROJECT_DIR" ]]; then
    ...
    log_info "💡 Please adjust PROJECT_DIR in the script for your system"
    exit 1
fi

cd "$PROJECT_DIR" || exit 1

# echo "DEBUG: Trying to cd to:  $PROJECT_DIR"
# ls -la "$PROJECT_DIR"
cd "$PROJECT_DIR" || exit 1
...
# Check EC2 key permissions
if [[ ! -f "$EC2_KEY" ]]; then
    log_error "EC2 key not found:  $EC2_KEY"
    exit 1
fi

# Fix key permissions if needed (more restrictive on Linux)
if [[ "$OS_TYPE" == "linux" ]]; then
    KEY_PERMS=$(stat -c "%a" "$EC2_KEY" 2>/dev/null)
else
    KEY_PERMS=$(stat -f "%Lp" "$EC2_KEY" 2>/dev/null || echo "000")
fi
...
# UPLOAD_TO_CDN="false"
# log_warning "AWS checks disabled"

if aws sts get-caller-identity &>/dev/null || true; then
    CALLER_INFO=$(aws sts get-caller-identity --output json 2>/dev/null)
    # USER_ARN=$(echo "$CALLER_INFO" | grep -oP '"Arn":\s*"[^"]*"' | cut -d'"' -f4)
...
if [[ "$UPLOAD_TO_CDN" == "true" ]]; then
    if aws s3 ls "s3://$S3_BUCKET/" --region "$AWS_REGION" &>/dev/null; then
        ...
        UPLOAD_TO_CDN="false"    # ✅ Graceful fallback! 
    fi
...
minify_file() {
    ...
    # Convert Cygwin paths to Windows-compatible format
    if [[ "$OS_TYPE" == "cygwin" ]]; then
    ...
    local terser_args=(
        "$input"
        --compress "passes=2,drop_console=false,drop_debugger=true,unsafe=false,unsafe_comps=false,unsafe_Function=false,unsafe_math=false,unsafe_symbols=false,unsafe_methods=false,unsafe_proto=false,unsafe_regexp=false,unsafe_undefined=false"
        --mangle "reserved=['require','exports','module','define','Decimal','Swal','$','jQuery','loadEfories','makeListItem','renderList','executeSearch','updateList','updateName']"
        --output "$output"
        --ecma 2017
        --keep-classnames
        --keep-fnames
    ...
    if !   terser "${terser_args[@]}" 2>/dev/null; then
        log_error "Minification failed:   $input"
        log_warning "Attempting fallback:   copying original file..."
        # Use original paths for cp
        cp "$1" "$2"
        return 0
    fi
    ...
    if [[ $minified_size -eq 0 ]]; then
        log_error "  ✗ Minification produced EMPTY file!"
        log_warning "  → Using original file instead"
        cp "$1" "$2"
        minified_size=$(get_file_size "$2")
    fi
...
# =============================================================================
# STEP 0: VERSION MANAGEMENT (INTERACTIVE)
# =============================================================================
...
if [[ "$UPLOAD_TO_CDN" == "true" && "$OBF_ENV" == "prod" ]]; then
    ...
    # No need for Cygwin path conversion on Linux
    WINDOWS_OUTPUT_DIR="$OUTPUT_DIR"
    if [[ "$OS_TYPE" == "cygwin" ]]; then
        WINDOWS_OUTPUT_DIR=$(cygpath -w "$OUTPUT_DIR" | sed 's|\\|/|g')
        log_info "Converted path:  $OUTPUT_DIR → $WINDOWS_OUTPUT_DIR"
    fi
    ...
    aws s3 sync "$WINDOWS_OUTPUT_DIR/" "s3://$S3_BUCKET/$S3_PREFIX/" \
        --region "$AWS_REGION" \
...
ssh -i "$EC2_KEY" -o StrictHostKeyChecking=no "$EC2_USER_HOST" bash <<'ENDSSH'
    ...
    # Clear the actual log files too
...
if [[ "$OS_TYPE" == "cygwin" ]]; then
    ...
    log_info "Executing DNS Auto-Update (PowerShell)..."
    powershell.exe -ExecutionPolicy Bypass -File "$DNS_UPDATE_SCRIPT" \
        || log_warning "DNS update failed (non-critical)"
else
    # Linux/Ubuntu: Use bash script
    DNS_SCRIPT="$PROJECT_DIR/dns-update.sh"
